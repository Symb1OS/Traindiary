...
public class TrainingDao implements TrainingImpl {
	...
	private static final String SELECT_ALL_TRAINING = "SELECT T.*, (SELECT  CASE WHEN COUNT(*) > 1 THEN 1 ELSE 0 END  FROM TRAINDIARY.TRAINING_EXERCISE TE WHERE T.TRAININGID = TE.TRAININGID) AS DESCRIPTION  FROM TRAINDIARY.TRAINING T WHERE USERNAME = ? AND MONTH(T.DATE) = ? AND YEAR(T.DATE) = ?";
