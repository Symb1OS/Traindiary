...
Ext.Boot = Ext.Boot || (function(emptyFn) {
	...
	var doc = document, _emptyArray = [], _config = {
		 ...
		 * added to script URL's to prevent caching. In debug builds, adding a
		 * "cache" or "disableCacheBuster" query parameter to the page's URL
		 ...
		 * loaded scripts, `true` to retain script element for browser debugger
		 * compatibility and improved load performance.
		 */
		preserveScripts : true,

		/*
		 * @cfg {String} charset Optional charset to specify encoding of dynamic
		 * content.
		 */
		charset : undefined
	},
	...
	Boot = {
		...
		/**
		 * The defult function that detects various platforms and sets tags in
		 * the platform map accrodingly. Examples are iOS, android, tablet, etc.
		 * 
		 * @param tags
		 *            the set of tags to populate
		 */
		detectPlatformTags : function() {
			var ua = navigator.userAgent, isMobile = _tags.isMobile = /Mobile(\/|\s)/
					.test(ua), isPhone, isDesktop, isTablet, touchSupported, isIE10, isBlackberry, element = document
					.createElement('div'), uaTagChecks = ['iPhone', 'iPod',
					'Android', 'Silk', 'Android 2', 'BlackBerry', 'BB', 'iPad',
					'RIM Tablet OS', 'MSIE 10', 'Trident', 'Chrome', 'Tizen',
					'Firefox', 'Safari', 'Windows Phone'], isEventSupported = function(
					name, tag) {
				if (tag === undefined) {
					tag = window;
				}
			...
			isPhone = (uaTags.iPhone || uaTags.iPod)
					|| (!uaTags.Silk && (uaTags.Android && (uaTags['Android 2'] || isMobile)))
					|| ((uaTags.BlackBerry || uaTags.BB) && uaTags.isMobile)
					|| (uaTags['Windows Phone']);

			isTablet = (!_tags.isPhone)
					&& (uaTags.iPad || uaTags.Android || uaTags.Silk
							|| uaTags['RIM Tablet OS'] || (uaTags['MSIE 10'] && /; Touch/
							.test(ua)));
		 ...
		 * is given, will return the config object @param {String} name The
		 * config property name @return {Object}
		 */
		...
		/*
		 * Set the configuration. @param {Object} config The config object to
		 * override the default values. @return {Ext.Boot} this
		 */
	...
	/*
	 * The request class encapsulates a series of Entry objects and provides
	 * notification around the completion of all Entries in this request.
	 */
	...
	Request.prototype = {
		...
		/*
		 * @private @param manifest @returns {*}
		 */
		createLoadOrderMap : function(loadOrder) {
		...
		/*
		 * @private @param index @param indexMap @returns {{}}
		 */
		getLoadIndexes : function(index, indexMap, loadOrder, includeUses,
				skipLoaded) {
			...
			while (!stop) {
				...
				// if we made a pass through the index map and didn't add
				// anything
				// then we can stop
				if (!added) {
					stop = true;
		...
		expandUrl : function(url, indexMap, includeUses, skipLoaded) {
			...
			if (loadOrder) {
				...
				for (i = 0; i < len; i++) {
					...
					if (item) {
						me.getLoadIndexes(item.idx, indexMap, loadOrder,
				...
				return me.getPathsFromIndexes(indexMap, loadOrder)
						.concat(unmapped);
			}
			return url;
		},
	...
	/*
	 * The Entry class is a token to manage the load and evaluation state of a
	 * particular url. It is used to notify all Requests interested in this url
	 ...
	 * using a cookie. @param {Boolean} disable True to disable the cache
	 * buster. @param {String} [path="/"] An optional path to scope the cookie.
	 */
 ...
 * at global scope, in others it will be in a function. @parma {String} code The
 * code to evaluate. @private @method
 */
