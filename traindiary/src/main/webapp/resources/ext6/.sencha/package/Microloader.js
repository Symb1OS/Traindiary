...
// scope to avoid duplicate Ext.Boot state. That check is after the initial Ext
// check
// to allow the sandboxing template to inject an appropriate Ext var and prevent
// the
// global detection.
...
Ext.Microloader = Ext.Microloader || (function() {
	 ...
	 * '_ext' is used to scope all the local storage keys that we internally by
	 * Ext 'location.pathname' is used to force each assets to cache by an
	 * absolute URL (/build/MyApp) (dev vs prod) 'url' is used to force each
	 * asset to cache relative to the page (app.json vs resources/app.css)
	 * 'profileId' is used to differentiate the builds of an application
	 * (neptune vs crisp) 'Microloader.appId' is unique to the application and
	 * will differentiate apps on the same host (dev mode running app watch
	 * against multiple apps)
	 */
	...
	LocalStorage = {
		clearAllPrivate : function(manifest) {
			if (_storage) {
				...
				var i, key, removeKeys = [], suffix = manifest.profile + '-'
						+ Microloader.appId, ln = _storage.length;
				for (i = 0; i < ln; i++) {
					key = _storage.key(i);
					// If key starts with the private key and the suffix is
					// present we can clear this entry
		...
		setAsset : function(key, content) {
			try {
				...
				if (_storage && e.code == e.QUOTA_EXCEEDED_ERR) {
					// <debug>
					_warn("LocalStorage Quota exceeded, cannot store " + key
							+ " locally");
					// </debug>
				}
			}
	...
	Asset.prototype = {
		shouldCache : function() {
			return _storage && this.assetConfig.update && this.assetConfig.hash
					&& !this.assetConfig.remote;
		},
	...
	var Manifest = function(cfg) {
		...
		this.cacheEnabled = this.content.cache
				? this.content.cache.enable
				: false;

		this.loadOrderMap = (this.loadOrder) ? Boot
				.createLoadOrderMap(this.loadOrder) : null;

		var tags = this.content.tags, platformTags = Ext.platformTags;
	...
	Manifest.prototype = {
		...
		processAssets : function(assets, type) {
			var results = [], ln = assets.length, i, assetConfig;
		...
		cache : function(content) {
			if (this.shouldCache()) {
				LocalStorage.setAsset(this.key, JSON.stringify(content
								|| this.content));
			}
			// <debug>
			else {
	 ...
	 * @private
	 */
	var _listeners = [], _loaded = false, Microloader = {
		init : function() {
		...
		run : function() {
			...
			if (typeof manifest === "string") {
				var extension = ".json", url = manifest.indexOf(extension) === manifest.length
						- extension.length ? manifest : manifest + ".json", key = getStorageKey(url), content = LocalStorage
						.retrieveAsset(key);

				// Manifest found in local storage, use this for immediate boot
				// except in PhantomJS environments for building.
				if (content) {
					// <debug>
					_debug("Manifest file, '" + url
					...
					// Manifest is not in local storage. Fetch it from the
					// server
				} else {
					Boot.fetch(url, function(result) {
						// <debug>
						_debug("Manifest file was not found in Local Storage, loading: "
								+ url);
						// </debug>
						manifest = new Manifest({
									url : url,
		...
		load : function(manifest) {
			...
			var assets = manifest.getAssets(), cachedAssets = [], asset, i, len, include, entry;
			...
			for (len = assets.length, i = 0; i < len; i++) {
				...
				if (include) {
					...
					if (manifest.shouldCache() && asset.shouldCache()) {
						// Asset already has content from localStorage,
						// instantly seed that into boot
						if (asset.content) {
							// <debug>
							_debug("Asset: "
							...
							entry = Boot.registerContent(
									asset.assetConfig.path, asset.type,
									asset.content);
							if (entry.evaluated) {
								_warn("Asset: "
										+ asset.assetConfig.path
			...
			// If any assets are using the caching system and do not have local
			// versions load them first via AJAX
			if (cachedAssets.length > 0) {
				Microloader.remainingCachedAssets = cachedAssets.length;
				while (cachedAssets.length > 0) {
					asset = cachedAssets.pop();
					// <debug>
					_debug("Preloading/Fetching Cached Assets from: "
							+ asset.assetConfig.path);
					// </debug>
					Boot.fetch(asset.assetConfig.path, (function(asset) {
								return function(result) {
									Microloader.onCachedAssetLoaded(asset,
											result);
								}
							})(asset));
				}
		...
		// Load the asset and seed its content into Boot to be evaluated in
		// sequence
		onCachedAssetLoaded : function(asset, result) {
			...
			if (!result.error) {
				checksum = Microloader.checksum(result.content,
				...
				// <debug>
				_debug("Checksum for Cached Asset: " + asset.assetConfig.path
						+ " is " + checksum);
				// </debug>
				Boot.registerContent(asset.assetConfig.path, asset.type,
						result.content);
				asset.updateContent(result.content);
				asset.cache();
			} else {
				_warn("There was an error pre-loading the asset '"
						+ asset.assetConfig.path
			...
			if (navigator.onLine !== false) {
				...
				if (window['addEventListener']) {
					window.addEventListener('online',
							Microloader.checkAllUpdates, false);
				}
			}
		},
		...
		checkForAppCacheUpdate : function() {
			...
			if (_cache.status === _cache.UPDATEREADY
				...
				_cache.addEventListener('noupdate',
						Microloader.onAppCacheNotUpdated);
				_cache.addEventListener('cached',
						Microloader.onAppCacheNotUpdated);
				_cache.addEventListener('updateready',
		...
		checkForUpdates : function() {
			...
			// </debug>
			Boot.fetch(Microloader.manifest.url,
		...
		onUpdatedManifestLoaded : function(result) {
			...
			if (!result.error) {
				var currentAssets, newAssets, currentAsset, newAsset, prop, assets, deltas, deltaPath, include, updatingAssets = [], manifest = new Manifest(
						{
				...
				// If the updated manifest has turned off caching we need to
				...
				if (!Microloader.manifest.is(manifest)) {
					...
					// Look through new assets for assets that do not exist or
					// assets that have different versions
					for (prop in newAssets) {
						newAsset = newAssets[prop];
						currentAsset = Microloader.manifest
								.getAsset(newAsset.assetConfig.path);
						include = Microloader.filterAsset(newAsset);

						if (include
								&& (!currentAsset || (newAsset.shouldCache() && (!currentAsset
										.is(newAsset))))) {
							// <debug>
							_debug("New/Updated Version of Asset: "
					...
					// Look through current assets for stale/old assets that
					// have been removed
					for (prop in currentAssets) {
						currentAsset = currentAssets[prop];
						newAsset = manifest
								.getAsset(currentAsset.assetConfig.path);
						...
						if (!include
								|| !newAsset
								|| (currentAsset.shouldCache() && !newAsset
										.shouldCache())) {
							// <debug>
							_debug("Asset: "
					...
					if (updatingAssets.length > 0) {
						...
						while (updatingAssets.length > 0) {
							...
							// Full Updates will simply download the file and
							// replace its current content
							if (newAsset.assetConfig.update === "full"
									|| !currentAsset) {

								// <debug>
								if (newAsset.assetConfig.update === "delta") {
									_debug("Delta updated asset found without current asset available: "
											+ newAsset.assetConfig.path
											+ " fetching full file");
								} else {
									_debug("Full update found for: "
											+ newAsset.assetConfig.path
											+ " fetching");
								}
								// </debug>

								// Load the asset and cache its its content into
								// Boot to be evaluated in sequence
								Boot.fetch(newAsset.assetConfig.path,
										(function(asset) {
											return function(result) {
												Microloader
								...
								deltaPath = deltas + "/"
										+ newAsset.assetConfig.path + "/"
										+ currentAsset.assetConfig.hash
										+ ".json";
								// Fetch the Delta Patch and update the contents
								// of the asset
								// <debug>
								_debug("Delta update found for: "
										+ newAsset.assetConfig.path
										+ " fetching");
								// </debug>
		...
		onFullAssetUpdateLoaded : function(asset, result) {
			...
			if (!result.error) {
				checksum = Microloader.checksum(result.content,
						asset.assetConfig.hash);
				// <debug>
				_debug("Checksum for Full asset: " + asset.assetConfig.path
						+ " is " + checksum);
				// </debug>
				if (!checksum) {
					// <debug>
					_debug("Full Update Asset: "
							+ asset.assetConfig.path
							+ " has failed checksum. This asset will be uncached for future loading");
					// </debug>

					// uncache this asset as there is a new version somewhere
				...
				// <debug>
				_debug("Error loading file at" + asset.assetConfig.path
						+ ". This asset will be uncached for future loading");
				// </debug>

				// uncache this asset as there is a new version somewhere that
				// has not been loaded.
				asset.uncache();
			}
		...
		onDeltaAssetUpdateLoaded : function(asset, oldAsset, result) {
			...
			if (!result.error) {
				...
				try {
					...
					checksum = Microloader.checksum(content,
							asset.assetConfig.hash);
					// <debug>
					_debug("Checksum for Delta Patched asset: "
							+ asset.assetConfig.path + " is " + checksum);
					// </debug>
					if (!checksum) {
						// <debug>
						_debug("Delta Update Asset: "
								+ asset.assetConfig.path
								+ " has failed checksum. This asset will be uncached for future loading");
						// </debug>

						// uncache this asset as there is a new version
						// somewhere that has not been loaded.
						asset.uncache();
					} else {
				...
				_warn("Error loading delta patch for " + asset.assetConfig.path
						+ " with hash " + oldAsset.assetConfig.hash
						+ " . This asset will be uncached for future loading");
		...
		// TODO: Make this all transaction based to allow for reverting if quota
		// is exceeded
		onAllUpdatedAssetsReady : function() {
			...
			if (Microloader.updatedManifest) {
				while (Microloader.removedAssets.length > 0) {
					...
					// <debug>
					_debug("Asset: " + asset.assetConfig.path
		...
		notifyUpdateReady : function() {
			if (Microloader.appCacheState !== 'checking'
					&& Microloader.updatedAssetsReady) {
				if (Microloader.appCacheState === 'updated'
					...
					Microloader.appUpdate = {
						...
						manifest : Microloader.updatedManifest
		...
		fireAppUpdate : function() {
			if (Ext.GlobalEvents) {
				// We defer dispatching this event slightly in order to let the
				// application finish loading
		...
		checksum : function(content, hash) {
			...
			var passed = true, hashLn = hash.length, checksumType = content
					.substring(0, 1);
		...
		parseResult : function(result) {
			...
			if ((result.exception || result.status === 0) && !Boot.env.phantom) {
				rst.error = true;
					...
					|| result.status === 304 || Boot.env.phantom
