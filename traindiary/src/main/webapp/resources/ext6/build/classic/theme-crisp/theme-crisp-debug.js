Ext.define('Ext.theme.neptune.Component', {
	...
	privates : {
		initStyles : function() {
			...
			if (me.dock) {
				// prevent the superclass method from setting the border style.
				// We want to
...
Ext.define('Ext.theme.neptune.layout.component.Dock', {
	...
	/**
	 * This table contains the border removal classes indexed by the sum of the
	 * edges to remove. Each edge is assigned a value: * `left` = 1 * `bottom` =
	 * 2 * `right` = 4 * `top` = 8
	 * 
	 * @private
	 */
	noBorderClassTable : [0,
			// TRBL
	...
	// 1111 = 15
	/**
	 * The numeric values assigned to each edge indexed by the `dock` config
	 * value.
	 * 
	 * @private
	 */
	...
	handleItemBorders : function() {
		...
		for (i = 0, ln = docked.length; i < ln; i++) {
			...
			if (item.ignoreBorderManagement) {
				// headers in framed panels ignore border management, so we do
				// not want
			...
			// mask can use += but edges must use |= because there can be
			// multiple items
...
Ext.define('Ext.theme.neptune.panel.Panel', {
			...
			initBodyBorder : function() {
				// The superclass method converts a truthy bodyBorder into a
				// number and sets
				// an inline border-width style on the body element. This
				// prevents that from
				// happening if borderBody === true so that the body will get
				// its border-width
				// the stylesheet.
...
Ext.define('Ext.theme.neptune.picker.Month', {
			...
			// Monthpicker contains logic that reduces the margins of the month
			// items if it detects
			// that the text has wrapped. This can happen in the classic theme
			// in certain
			// locales such as zh_TW. In order to work around this, Month picker
			// measures
			// the month items to see if the height is greater than
			// "measureMaxHeight".
			// In neptune the height of the items is larger, so we must increase
			// this value.
			// While the actual height of the month items in neptune is 24px, we
			// will only
			// determine that the text has wrapped if the height of the item
			// exceeds 36px.
			// this allows theme developers some leeway to increase the month
			// item size in
