Ext.define('FV.controller.Articles', {
	...
	previewArticle : function(grid, articles) {
		var article = articles[0], articlePreview = this.getArticlePreview();
	...
	openLinkMobile : function(url) {
		var link = Ext.getDom('hidden_link'), clickEvent = document
				.createEvent('Event');
	...
	openArticle : function(btn) {
		var open, url = btn.up('articlepreview').article.get('link'), deviceType = Ext.os.deviceType;

		if (Ext.os.name !== 'iOS' && deviceType !== 'Desktop') {
			this.openLinkMobile(url);
		} else {
			open = window.open(url); // Workaround for iOS! Assigning result
			// to variable brings up prompt on iOS.
			// On desktop it should return Window
			// object
			if (!open) {
				// If null or undefined, then we inform user that popup blocker
				// should be disabled
				Ext.Msg
						.alert(
								'Popups disabled!',
								'To view selected article in the new window disable Pop-up blocker or choose Allow, when promted by your device.<br><br>');
			}
		}
	},
	...
	loadArticles : function(articles) {
		var viewer = this.getViewer(), toAdd = [], tab, id;
	...
	loadArticle : function(view, article) {
		var viewer = this.getViewer(), title = article.get('title'), articleId = article.id;
