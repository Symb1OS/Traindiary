...
Ext.define('Books.view.review.List', {
	...
	initComponent : function() {
		this.dataview = Ext.create('Ext.view.View', {
			...
			tpl : new Ext.XTemplate(
					...
					'<div class="comment">{comment}</div>', '</div>', '</tpl>',
					{
						stars : function(values) {
							var res = [], extension = Ext.isIE6 ? 'gif' : 'png', i = 0;

							// print out the stars for each of the ratings
							for (; i < values.rating; i++) {
								res.push('<img src="./resources/images/star.',
										extension, '" />');
							}

							// print out transparent stars for the rest (up to
							// 5)
							while (i < 5) {
								res
										.push(
												'<img src="./resources/images/star_no.',
												extension, '" />');
								i++;
							}
	...
	/**
	 * Used to bind a store to this dataview. Delegates to bindStore and also
	 * shows this view
	 * 
	 * @param {Ext.data.Model}
	 *            record The record to bind
	 * @param {Ext.data.Store}
	 *            store The reviews store used by the application
	 */
