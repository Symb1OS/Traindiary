...
Ext.define('Ext.calendar.dd.DayDropZone', {
	...
	onNodeOver : function(n, dd, e, data) {
		var dt, box, endDt, text = this.createText, curr, start, end, evtEl, dayCol;
		if (data.type == 'caldrag') {
			if (!this.dragStartMarker) {
				// Since the container can scroll, this gets a little tricky.
				// There is no el in the DOM that we can measure by default
				// since
				// the box is simply calculated from the original drag start (as
				// opposed
				// to dragging or resizing the event where the orig event box is
				// present).
				// To work around this we add a placeholder el into the DOM and
				// give it
				// the original starting time's box so that we can grab its
				// updated
				// box measurements as the underlying container scrolls up or
				// down.
			...
			box.height = Math
					.ceil(Math.abs(e.xy[1] - box.y) / n.timeBox.height)
					* n.timeBox.height;
			...
			this.dragStartDate = Ext.calendar.util.Date.min(this.dragCreateDt,
					curr);
			this.dragEndDate = endDt
					|| Ext.calendar.util.Date.max(this.dragCreateDt, curr);

			dt = Ext.Date.format(this.dragStartDate, 'g:ia-')
					+ Ext.Date.format(this.dragEndDate, 'g:ia');
		} else {
			...
			if (data.type == 'eventresize') {
				...
				box.x = dayCol.getX();
				box.height = Math.ceil(Math.abs(e.xy[1] - box.y)
						/ n.timeBox.height)
						* n.timeBox.height;
				if (e.xy[1] < box.y) {
					box.y -= box.height;
				...
				dt = Ext.Date.format(start, 'g:ia-')
						+ Ext.Date.format(end, 'g:ia');
