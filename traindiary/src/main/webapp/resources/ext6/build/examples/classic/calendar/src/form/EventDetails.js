 ...
 *          </p>
 *          <p>
 *          This is pretty much a standard form that is simply pre-configured
 *          for the options needed by the calendar components. It is also
 *          configured to automatically bind records of type
 *          {@link Ext.calendar.EventRecord} to and from the form.
 *          </p>
 *          <p>
 *          This form also provides custom events specific to the calendar so
...
Ext.define('Ext.calendar.form.EventDetails', {
	...
	requires : ['Ext.calendar.form.field.DateRange',
	 ...
	 * @param {Ext.calendar.form.EventDetails}
	 *            this
	...
	initComponent : function() {
		...
		var leftFields = [this.titleField, this.dateRangeField,
				this.reminderField], rightFields = [this.notesField,
	...
	updateRecord : function() {
		var dates = this.dateRangeField.getValue(), M = Ext.calendar.data.EventMappings, rec = this.activeRecord, fs = rec.fields, dirty = false;
		...
		// that call. We need the dirty status, plus currently the
		// DateRangeField
		// does not map directly to the record values, so for now we'll
		// duplicate
		// the setter logic here (we need to be able to pick up any custom-added
		// fields generically). Need to revisit this later and come up with a
		// better solution.
	...
	setStartDate : function(d) {
		var me = this, duration = me.dateRangeField.getDuration();

		me.dateRangeField.setDT(d, 'start');

		// Set the end time to keep the duration the same
		me.dateRangeField.setDT(new Date(me.dateRangeField.getDT('start')
						.getTime()
						+ duration), 'end');
	},
