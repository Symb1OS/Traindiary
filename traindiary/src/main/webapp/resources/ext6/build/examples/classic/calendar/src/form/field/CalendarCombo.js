 ...
 *          <p>
 *          A custom combo used for choosing from the list of available
 *          calendars to assign an event to.
 *          </p>
 *          <p>
 *          This is pretty much a standard combo that is simply pre-configured
 *          for the options needed by the calendar components. The default
...
Ext.define('Ext.calendar.form.field.CalendarCombo', {
	...
	getListItemTpl : function(displayField) {
		return '<div class="x-combo-list-item ext-color-{'
				+ Ext.calendar.data.CalendarMappings.CalendarId.name
	...
	/*
	 * @private Value can be a data value or record, or an array of values or
	 * records.
	 */
	getStyleClass : function(value) {
		...
		if (!Ext.isEmpty(val)) {
			...
			return 'ext-color-'
					+ (val.data
							? val.data[Ext.calendar.data.CalendarMappings.CalendarId.name]
							: val);
		}
		return '';
	},
	...
	setValue : function(value) {
		...
		if (this.wrap && value) {
			var currentClass = this.getStyleClass(this.getValue()), newClass = this
					.getStyleClass(value);
