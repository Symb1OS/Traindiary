 ...
 *          <p>
 *          A combination field that includes start and end dates and times, as
 *          well as an optional all-day checkbox.
 *          </p>
 * @constructor
 * @param {Object}
...
Ext.define('Ext.calendar.form.field.DateRange', {
	...
	/**
	 * @cfg {String} toText The text to display in between the date/time fields
	 *      (defaults to 'to')
	 */
	toText : 'to',
	/**
	 * @cfg {String} allDayText The text to display as the label for the all day
	 *      checkbox (defaults to 'All day')
	 */
	allDayText : 'All day',
	/**
	 * @cfg {String/Boolean} singleLine `true` to render the fields all on one
	 *      line, `false` to break the start date/time and end date/time into
	 *      two stacked rows of fields to preserve horizontal space (defaults to
	 *      `true`).
	 */
	singleLine : true,
	/**
	 * @cfg {String} dateFormat The date display format used by the date fields
	 *      (defaults to 'n/j/Y')
	 */
	dateFormat : 'n/j/Y',
	/**
	 * @cfg {String} timeFormat The time display format used by the time fields.
	 *      By default the DateRange uses the {@link Ext.Date.use24HourTime}
	 *      setting and sets the format to 'g:i A' for 12-hour time (e.g., 1:30
	 *      PM) or 'G:i' for 24-hour time (e.g., 13:30). This can also be
	 *      overridden by a static format string if desired.
	 */
	...
	initComponent : function() {
		...
		if (me.singleLine) {
			...
			me.items = [{
				...
				items : [me.getStartDateConfig(), me.getStartTimeConfig(),
						me.getDateSeparatorConfig()]
			}, {
				xtype : 'container',
				layout : me.fieldLayout,
				items : [me.getEndDateConfig(), me.getEndTimeConfig(),
						me.getAllDayConfig()]
			}];
		}
	...
	initRefs : function() {
		...
		me.startDate.isValid = me.endDate.isValid = function() {
			var me = this, valid = Ext.isDate(me.getValue());
			if (!valid) {
				me.focus();
			}
			return valid;
	...
	getFieldConfigs : function() {
		...
		return [me.getStartDateConfig(), me.getStartTimeConfig(),
				...
				me.getEndDateConfig(), me.getAllDayConfig()];
	},
	...
	getDuration : function() {
		var me = this, start = me.getDT('start'), end = me.getDT('end');
	...
	isSingleLine : function() {
		...
		if (me.calculatedSingleLine === undefined) {
			if (me.singleLine == 'auto') {
				var ownerCtEl = me.ownerCt.getEl(), w = me.ownerCt.getWidth()
						- ownerCtEl.getPadding('lr'), el = ownerCtEl
						.down('.x-panel-body');
				...
				me.calculatedSingleLine = w <= me.singleLineMinWidth
						? false
						: true;
			} else {
				me.calculatedSingleLine = me.singleLine !== undefined
						? me.singleLine
						: true;
			}
		}
	...
	checkDates : function(type, startend) {
		var me = this, startField = me.down('#' + me.id + '-start-' + type), endField = me
				.down('#' + me.id + '-end-' + type), startValue = me
				.getDT('start'), endValue = me.getDT('end');
	...
	/**
	 * Returns an array containing the following values in order:<div
	 * class="mdetail-params">
	 * <ul>
	 * <li><b><code>DateTime</code></b> : <div class="sub-desc">The start
	 * date/time</div></li>
	 * <li><b><code>DateTime</code></b> : <div class="sub-desc">The end
	 * date/time</div></li>
	 * <li><b><code>Boolean</code></b> : <div class="sub-desc">True if the
	 * dates are all-day, false if the time values should be used</div></li>
	 * <ul>
	 * </div>
	 * 
	 ...
	 */
	getValue : function() {
		var eDate = Ext.calendar.util.Date, start = this.getDT('start'), end = this
				.getDT('end'), allDay = this.allDay.getValue();
	 ...
	 *            v The value(s) to set into the field. Valid types are as
	 *            follows:<div class="mdetail-params">
	 *            <ul>
	 *            <li><b><code>Array</code></b> : <div class="sub-desc">An
	 *            array containing, in order, a start date, end date and all-day
	 *            flag. This array should exactly match the return type as
	 *            specified by {@link #getValue}.</div></li>
	 *            <li><b><code>DateTime</code></b> : <div
	 *            class="sub-desc">A single Date object, which will be used for
	 *            both the start and end dates in the range. The all-day flag
	 *            will be defaulted to false.</div></li>
	 *            <li><b><code>Object</code></b> : <div class="sub-desc">An
	 *            object containing properties for StartDate, EndDate and
	 *            IsAllDay as defined in {@link Ext.calendar.data.EventMappings}.</div></li>
	 *            <ul>
	 *            </div>
	 */
	setValue : function(v) {
		...
		if (Ext.isArray(v)) {
			...
			this.setDT(v[Ext.calendar.data.EventMappings.StartDate.name],
					'start');
			if (!this.setDT(v[Ext.calendar.data.EventMappings.EndDate.name],
					'end')) {
				this.setDT(v[Ext.calendar.data.EventMappings.StartDate.name],
						'end');
			}
			this.allDay
					.setValue(!!v[Ext.calendar.data.EventMappings.IsAllDay.name]);
		}
	},
