Ext.define('Ext.calendar.util.Date', {
	...
	diffDays : function(start, end) {
		var day = 1000 * 60 * 60 * 24, clear = Ext.Date.clearTime, diff = clear(
				end, true).getTime()
				- clear(start, true).getTime();
	...
	copyTime : function(fromDt, toDt) {
		...
		dt.setHours(fromDt.getHours(), fromDt.getMinutes(),
				fromDt.getSeconds(), fromDt.getMilliseconds());
	...
	/**
	 * Adds time to the specified date and returns a new Date instance as the
	 * result (does not alter the original date object). Time can be specified
	 * in any combination of milliseconds to years, and the function
	 * automatically takes leap years and daylight savings into account. Some
	 * syntax examples:<code><pre>
	 * var now = new Date();
	 * 
	 * // Add 24 hours to the current date/time:
	 * var tomorrow = Extensible.Date.add(now, {
	 * 			days : 1
	 * 		});
	 * 
	 ...
	 * 
	 * @param {Date}
	 *            dt The starting date to which to add time
	 * @param {Object}
	 *            o A config object that can contain one or more of the
	 *            following properties, each with an integer value: - millis -
	 *            seconds - minutes - hours - days - weeks - months - years
	 * 
	 * You can also optionally include the property "clearTime: true" which will
	 * perform all of the date addition first, then clear the time value of the
	 * final date before returning it.
	 * @return {Date} A new date instance containing the resulting date/time
	 *         value
	 */
	add : function(dt, o) {
		if (!o) {
		...
		var ExtDate = Ext.Date, dateAdd = ExtDate.add, newDt = ExtDate
				.clone(dt);
