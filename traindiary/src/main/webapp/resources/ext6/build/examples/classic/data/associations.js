Ext.require(['Ext.data.*', 'Ext.panel.Panel', 'Ext.layout.container.Card',
...
Ext.define('CustomerGrid', {
	...
	loadOrders : function() {
		var rec = this.active, name = rec.get('name'), owner = this.ownerCt, orders;
...
Ext.define('OrderGrid', {
			...
			loadItems : function() {
				var rec = this.active, owner = this.ownerCt, orderitems;

				orderitems = rec.orderItems();
				if (orderitems.isLoading()) {
					Logger.log('Begin loading order items - ' + rec.getId(),
							true);
				}
				orderitems.on('load', function() {
							Logger.log('Order items loaded - ' + rec.getId(),
									false);
						});
				owner.add({
...
Ext.define('OrderItemGrid', {
	...
	onLoadClick : function() {
		...
		new ItemLoader({
					...
					height : 400,
					modal : true,
					title : this.title.replace('Order Items', 'Order Item '
									+ id),
