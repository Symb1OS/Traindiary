...
Ext.define('BigDataSimlet', {
	...
	getData : function(ctx) {
		var me = this, data = me.data, groupSize = ctx.params.groupSize || 1, group;

		if (data.length != me.numRecords || me.lastNumColumns != me.numFields
				|| groupSize !== me.groupSize) {
...
Ext.onReady(function() {
	...
	Ext.define('StoreInfo', {
				...
				prefetch : function(options) {
					...
					if (!this.pageCached(page) && !this.pagePending(page)) {
						logPanel.log('Prefetch rows ' + options.start + '-'
				...
				onProxyPrefetch : function(operation) {
					this.callParent(arguments);
					logPanel.log('Prefetch returned ' + operation.getStart()
							+ '-'
							+ (operation.getStart() + operation.getLimit()));
				}
			});
	...
	var simlet = new BigDataSimlet(), center, storeCount = 0, summaryTypes = [
			'count', 'min', 'max', 'sum', 'average'], summaryTypeNames = [
	...
	function createStore(numFields, buffered, pageSize, groupSize) {
		var fields = [], i, storeConfig, result, pageCache;
		...
		result = Ext.create(buffered
						? 'Ext.data.BufferedStore'
						: 'Ext.data.Store', storeConfig);
		pageCache = result.getData();

		// Inject logging into the LruCache
		if (pageCache instanceof Ext.util.LruCache) {
			pageCache.prune = Ext.Function.createSequence(pageCache.prune,
					function() {
						logPanel.log('Page cache contains pages '
								+ this.getKeys().join(',')
								+ '<br>&#160;&#160;&#160;&#160;'
								+ this.getCount() + ' records in cache');
					});
		}
				...
				groupHeaderTpl : [
						...
						'</tpl>', {
							groupName : function(group) {
								...
								return groupColumnRenderer(rowIdx, null, null,
										rowIdx, 1);
							}
						}],
				id : 'field0Grouping'
	...
	var store = createStore(10), grid = createGrid({
				store : store
			});
	...
	function makeLabel(ns, cls, name) {
		...
		return '<a href="' + docs + '/docs/#!/api/' + ns + '.' + cls + '-cfg-'
				+ name + '" target="docs" tabIndex="-1">' + cls + ' ' + name
				+ '</a>';
	}
	...
	var controls = Ext.create('Ext.form.Panel', {
		...
		items : [{
					...
					xtype : 'numberfield',
					fieldLabel : makeLabel('Ext.grid', 'BufferedRenderer',
							'trailingBufferZone'),
					...
					listeners : {
						change : function(fld, newVal, oldVal) {
							var nfeField = controls
									.down('#scrollerNumFromEdge');
					...
					listeners : {
						change : function(cb, checked) {
							var pageSize = controls.down('#pageSize'), storeTrailingBufferZone = controls
									.down('#storeTrailingBufferZone'), storeLeadingBufferZone = controls
									.down('#storeLeadingBufferZone'), purgePageCount = controls
									.down('#purgePageCount'), scrollToLoadBuffer = controls
									.down('#scrollToLoadBuffer');
					...
					xtype : 'numberfield',
					fieldLabel : makeLabel('Ext.data', 'Store',
							'trailingBufferZone'),
	...
	function groupColumnRenderer(v, metadata, record, rowIdx, colIdx, store,
			view) {
		var groupSize = simlet.groupSize, group = Math
				.floor((rowIdx + groupSize) / groupSize), groupStart = (group - 1)
				* groupSize + 1, groupEnd = groupStart + (groupSize - 1);

		return 'rows ' + groupStart + ' to ' + groupEnd;
	}

	function scrollTopToBottom(b, maxScroll) {
		var view = grid.view, el = view.el, br = view.bufferedRenderer, oldSynchronousRender = br.oldSynchronousRender, scrollQuantum = 30, start = new Date()
				.getTime(), nextScroll = function() {
			...
			if (newPos >= maxScroll) {
				...
				logPanel.log('Scroll took ' + (new Date().getTime() - start)
						+ ' ms');
		...
		br.handleViewScroll = Ext.Function.createSequence(br.handleViewScroll,
				nextScroll);
		nextScroll();
	}
	...
	function initializeGrid() {
		var numFields = controls.down('#numFields').getValue(), columns = [{
					xtype : 'rownumberer',
					width : 50,
					sortable : false
				}], i, rowCount = controls.down('#rowCount').getValue(), groupSize = grid.ownerCt
				.down('#groupSize').getValue()
				|| 0, buffered = controls.down('#buffered').getValue(), columnSpec;
		...
		for (i = 0; i < numFields; ++i) {
			columnSpec = {
				...
				align : 'right',
				summaryRenderer : function(value, cellValues, record,
						recordIndex, colIdx, store, view) {
					cellValues.style = 'text-align:right';
					return summaryTypeNames[(colIdx - 1) % 5] + ': '
		...
		store = createStore(numFields, buffered, controls.down('#pageSize')
						.getValue(), groupSize);

		store.trailingBufferZone = controls.down('#storeTrailingBufferZone')
				.getValue();
		store.leadingBufferZone = controls.down('#storeLeadingBufferZone')
				.getValue();
		store.data.maxSize = store.purgePageCount = controls
				.down('#purgePageCount').getValue();

		grid = createGrid({
					title : 'Random data (' + simlet.numRecords + ' records)',
					store : store,
					columns : columns,
					numFromEdge : controls.down('#scrollerNumFromEdge')
							.getValue(),
					trailingBufferZone : controls
							.down('#scrollerTrailingBufferZone').getValue(),
					leadingBufferZone : controls
							.down('#scrollerLeadingBufferZone').getValue()
				});
		...
		if (buffered) {
			// Load the first page. It will be diverted through the prefetch
			// buffer.
			store.loadPage(1);
		} else {
			store.load({
				...
				render : {
					fn : function(p) {
						Ext.on('idle', function() {
							p.header
									.removeCls('x-docked-noborder-left x-docked-noborder-right x-docked-noborder-top');
							p.updateLayout({
										isRoot : true
									});
						}, null, {
			...
			items : [controls, logPanel]
					...
					bbar : [{
						...
						handler : function(scrollButton) {
							var v = grid.view, el = v.el;

							if (v.bufferedRenderer
									.hasOwnProperty('handleViewScroll')) {
								delete v.bufferedRenderer.handleViewScroll;
								scrollButton
										.setText(scrollButton.initialConfig.text);
							} else {
								scrollButton.setText('Stop scroll');
								v.el.dom.scrollTop = 0;
								Ext.defer(scrollTopToBottom, controls
												.down('#latency').getValue()
												+ 200, null, [
												scrollButton,
												el.dom.scrollHeight
														- el.dom.clientHeight]);
						...
						handler : function(scrollButton) {
							...
							if (v.bufferedRenderer
								...
								Ext.defer(scrollBottomToTop, controls
												.down('#latency').getValue()
												+ 200, null, [
												scrollButton,
												el.dom.scrollHeight
