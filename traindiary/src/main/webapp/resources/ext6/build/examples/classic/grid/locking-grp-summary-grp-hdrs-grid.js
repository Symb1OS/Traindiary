...
Ext.require(['Ext.grid.*', 'Ext.layout.container.Border', 'Ext.data.*',
		'Ext.form.field.Number', 'Ext.form.field.Date',
		'Ext.tip.QuickTipManager', 'Ext.ux.DataTip']);
	...
	var grid = Ext.create('Ext.grid.Panel', {
		...
		listeners : {
			beforeshowtip : function(grid, tip, data) {
				var cellNode = tip.triggerEvent.getTarget(tip.view
						.getCellSelector());
				if (cellNode) {
					data.colName = tip.view.headerCt.columnManager
							.getHeaderAtIndex(cellNode.cellIndex).text;
				}
			}
		},
		selModel : {
		...
		dockedItems : [{
			...
			items : [{
				...
				pressed : true,
				handler : function() {
					grid.lockedGrid.getView().getFeature('group')
							.toggleSummaryRow();
				}
			}]
		}],
		...
		columns : [{
			...
			columns : [{
						...
						renderer : function(value, metaData, record, rowIdx,
								colIdx, store, view) {
							return Ext.util.Format.usMoney(record
									.get('estimate')
									* record.get('rate'));
						},
