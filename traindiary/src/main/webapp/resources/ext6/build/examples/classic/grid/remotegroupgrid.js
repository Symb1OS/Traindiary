Ext.require(['Ext.ux.ajax.JsonSimlet', 'Ext.ux.ajax.SimManager', 'Ext.data.*',
		'Ext.grid.*']);
Ext.onReady(function() {
	...
	Ext.ux.ajax.SimManager.init({
		...
		'readRestaurants' : {
			data : data,
			stype : 'json',
			getData : Ext.Function.createInterceptor(
					Ext.ux.ajax.JsonSimlet.prototype.getData, function() {
	...
	var restaurants = Ext.create('Ext.data.Store', {
		...
		listeners : {
			...
			groupchange : function() {
				grid.down('[text=Clear Grouping]').setDisabled(!restaurants
						.isGrouped());
			},

			// Upon successful add of a new restaurant, invoke the server to
			// sort and group
	...
	var newRestaurantDialog = Ext.create('Ext.window.Window', {
				...
				// Make the Window out of a <form> el so that the <button
				// type="submit"> will invoke its handler upon CR
				autoEl : 'form',
				width : 400,
				bodyPadding : 5,
				layout : 'anchor',
				...
				buttons : [{
					...
					handler : function() {
						var newRec = new Restaurant({
									name : newRestaurantDialog
											.down('[name=name]').getValue(),
									cuisine : newRestaurantDialog
											.down('[name=cuisine]').getValue()
								});
