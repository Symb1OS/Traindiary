Ext.define('Router.controller.Navigation', {
	...
	config : {
		...
		routes : {
			'main-tabs:id:subid' : {
				...
				conditions : {
					// take control of the :id & :subid parameters, make them
					// optional but delimited by colon
					':id' : '(?:(?::){1}([%a-zA-Z0-9\-\_\s,]+))?',
	...
	onTabChange : function(tabPanel, newItem) {
		var id = newItem.getId(), child = newItem.child('tabpanel'), subid = '', hash = 'main-tabs:'
				+ id;
	...
	onChildTabChange : function(tabPanel, newItem) {
		var parentTab = tabPanel.up(), parentId = parentTab.getId(), hash = 'main-tabs:'
	...
	beforeShowTab : function(id, subid, action) {
		...
		if (child) {
			...
			Ext.Msg.alert('Tab Not Found', 'Tab with id or index "<b>' + id
							+ '</b>" was not found!');
	...
	showTab : function(id, subid) {
		var tabPanel = this.getTabPanel(), child, childTabPanel;
