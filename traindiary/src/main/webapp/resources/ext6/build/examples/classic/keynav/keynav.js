Ext.require(['Ext.XTemplate', 'Ext.util.KeyNav', 'Ext.fx.*']);
...
Ext.define('Board', {
			...
			render : function(el) {
				...
				var tpl = Ext.create('Ext.XTemplate', '<tpl for=".">',
						'<div class="square">{.}</div>', '<tpl if="xindex % '
								+ this.size
								+ ' === 0"><div class="x-clear"></div></tpl>',
						'</tpl>'), data = [], i = 0, max = this.size
						* this.size;
			...
			setActive : function(x, y) {
				var xy = this.constrain(x, y), cell = this.cells.item(this
						.getIndex(xy));
...
Ext.onReady(function() {
			...
			var nav = Ext.create('Ext.util.KeyNav', Ext.getDoc(), {
						...
						home : function() {
							cls = Ext.String.toggle(cls, 'active-green',
									'active-red');
