...
Ext.onReady(function() {
	 ...
	 * App.RadarStore
	 * 
	 * @extends Ext.data.Store This is a specialized Data Store with dynamically
	 *          generated fields data reformating capabilities to transform
	 *          Employee and Review data into the format required by the Radar
	 *          Chart.
	 * 
	 * The constructor demonstrates dynamically generating store fields.
	 * populateReviewScores() populates the store using records from the
	 * reviewStore which holds all the employee review scores.
	 * 
	 * calculateAverageScores() iterates through each metric in the review and
	 * calculates an average across all available reviews.
	 * 
	 * Most of the actual data population and updates done by
	 * addUpdateRecordFromReviews() and removeRecordFromReviews() called when
	 * add/update/delete events are triggered on the ReviewStore.
	 */
	Ext.define('App.RadarStore', {
				...
				constructor : function(config) {
					config = config || {};
					var dynamicFields = ['metric', 'avg']; // initalize the
					// non-dynamic
					// fields first

					employeeStore.each(function(record) { // loops through all
								// the employees to
								// setup the dynamic
								// fields
					...
					Ext.apply(config, {
								storeId : 'radarStore', // let's us look it up
								// later using
								// Ext.data.StoreMgr.lookup('radarStore')
				...
				addUpdateRecordFromReviews : function(reviews) {
					...
					Ext.Array.each(reviews, function(review, recordIndex, all) { // add
								// a
								// new
								// radarStore
								// record
								...
								// review
								// record
								var eid = 'eid_' + review.get('employee_id'); // creates
								// a
								// unique
								// id
								// for
				 ...
				 * We use this to create the average series always shown in the
				 * Radar Chart.
				 */
				calculateAverageScores : function() {
					var me = this; // keeps the store in scope during
					// Ext.Array.each
					var reviewStore = Ext.data.StoreMgr.lookup('reviewStore');

					var Review = Ext.ModelMgr.getModel('Review');

					Ext.Array.each(Review.prototype.fields.keys, function(
							fieldName) { // loop through the Review model
								// fields and calculate average
								// scores
								if (fieldName !== "employee_id"
										&& fieldName !== "review_date") { // ignore
									// non-score
									// fields
									var avgScore = Math.round(reviewStore
											.average(fieldName)); // takes
									// advantage
									// of
									// Ext.data.Store.average()
									var record = me.findRecord('metric',
											fieldName);
				...
				populateReviewScores : function() {
					var reviewStore = Ext.data.StoreMgr.lookup('reviewStore');
					this.addUpdateRecordFromReviews(reviewStore.data.items); // add
					// all
					// the
					// review
					// records
	...
	/**
	 * Creates an instance of App.RadarStore here so we here so we can re-use it
	 * during the life of the app. Otherwise we'd have to create a new instance
	 * everytime refreshRadarChart() is run.
	 */
	...
	var reviewStore = new Ext.data.Store({
				...
				listeners : {
					add : function(store, records, storeIndex) {
						...
						if (radarStore) { // only add records if an instance
							// of the rardarStore already exists
							radarStore.addUpdateRecordFromReviews(records); // add
							// a
							// new
							// radarStore
							// records
					...
					remove : function(store, records, storeIndex) {
						...
						Ext.data.StoreMgr.lookup('radarStore')
								.removeRecordFromReviews(records);
						refreshRadarChart();
					} // end remove listener
				}
			});
	 ...
	 * 
	 * @extends Ext.chart.Chart This is a specialized Radar Chart which we use
	 *          to display employee performance reviews.
	 * 
	 * The class will be registered with an xtype of 'performanceradar'
	 */
	Ext.define('App.PerformanceRadar', {
				extend : 'Ext.chart.Chart',
				alias : 'widget.performanceradar', // register xtype
				// performanceradar
				constructor : function(config) {
					config = config || {};

					this.setAverageSeries(config); // make sure average is
					// always present
					...
					App.PerformanceRadar.superclass.constructor.call(this,
							config);
				...
				setAverageSeries : function(config) {
					...
					if (config.series) {
						config.series.push(avgSeries); // if a series is passed
						// in then append the
						// average to it
					} else {
						config.series = [avgSeries]; // if a series isn't
						// passed just create
						// average
					}
				}
	 ...
	 * App.EmployeeDetail
	 * 
	 * @extends Ext.Panel This is a specialized Panel which is used to show
	 *          information about an employee and the reviews we have on record
	 *          for them.
	 * 
	 * This demonstrates adding 2 custom properties (tplMarkup and
	 * startingMarkup) to the class. It also overrides the initComponent method
	 * and adds a new method called updateDetail.
	 * 
	 * The class will be registered with an xtype of 'employeedetail'
	 */
	Ext.define('App.EmployeeDetail', {
				extend : 'Ext.panel.Panel',
				// register the App.EmployeeDetail class with an xtype of
				// employeedetail
				...
				buttons : [{
							...
							handler : function(btn, eventObj) {
								...
								if (form.isValid()) {
									...
									var employeeStore = Ext.data.StoreMgr
											.lookup('employeeStore');
									var currentEmployee = employeeStore
											.findRecord('id',
													vals['employee_id']);

									// look up id for this employee to see if
									// they already exist
									if (vals['employee_id'] && currentEmployee) {
										currentEmployee.set('first_name',
												vals['first_name']);
										currentEmployee.set('last_name',
												vals['last_name']);
										currentEmployee.set('title',
												vals['title']);

										var currentReview = Ext.data.StoreMgr
												.lookup('reviewStore')
												.findRecord('employee_id',
														vals['employee_id']);
										currentReview.set('review_date',
												vals['review_date']);
										currentReview.set('attendance',
												vals['attendance']);
										currentReview.set('attitude',
												vals['attitude']);
										currentReview.set('communication',
												vals['communication']);
										currentReview.set('excellence',
												vals['excellence']);
										currentReview.set('skills',
												vals['skills']);
										currentReview.set('teamwork',
												vals['teamwork']);
									} else {
										var newId = employeeStore.getCount()
												+ 1;
										...
										Ext.data.StoreMgr.lookup('reviewStore')
												.add({
	...
	function refreshRadarChart(employees) {
		employees = employees || []; // in case its called with nothing we'll
		// at least have an empty array
		var existingRadarChart = Ext.getCmp('radarchart'); // grab the radar
		// chart component
		// (used down below)
		var reportsPanel = Ext.getCmp('reportspanel'); // grab the reports
		// panel component (used
		// down below)
		var dynamicSeries = []; // setup an array of chart series that we'll
		// create dynamically

		for (var index = 0; index < employees.length; index++) {
			var fullName = employees[index].get('first_name') + ' '
					+ employees[index].get('last_name');
		...
		var newRadarChart = new App.PerformanceRadar({
	...
	function refreshEmployeeDetails(employees) {
		...
		for (var index = 0; index < employees.length; index++) {
			var templateData = Ext.applyIf(employees[index].data, reviewStore
							.findRecord('employee_id', employees[index]
											.get('id')).data);
			var employeePanel = new App.EmployeeDetail({
				title : employees[index].get('first_name') + ' '
						+ employees[index].get('last_name'),
				data : templateData
	...
	var viewport = new Ext.container.Viewport({
		...
		items : [{
			...
			tbar : [{
						...
						iconCls : 'add',
						handler : function() { // display a window to add a new
							// employee
				...
				columns : [{
							...
							items : [{
								...
								handler : function(grid, rowIndex, colIndex) {
									var employee = grid.getStore()
											.getAt(rowIndex);
									var review = reviewStore.findRecord(
											'employee_id', employee.get('id'));
									var win = new App.ReviewWindow({
												hidden : true
											});
								...
								handler : function(grid, rowIndex, colIndex) {
									Ext.Msg
											.confirm(
													...
													function(choice) {
														if (choice === 'yes') {
															var reviewStore = Ext.data.StoreMgr
																	.lookup('reviewStore');

															var employee = grid
																	.getStore()
																	.getAt(rowIndex);
															var reviewIndex = reviewStore
																	.find(
																			'employee_id',
																			employee
																					.get('id'));
			...
			layout : 'fit',
			items : [{
				xtype : 'performanceradar' // this instantiates a
					// App.PerformanceRadar
					// object
				}]
		}]
