/*
 * ! Ext JS Library 3.3.1 Copyright(c) 2006-2014 Sencha Inc.
...
Ext.ux.StartMenu = Ext.extend(Ext.menu.Menu, {
			 ...
			 * @param {String}
			 *            position (optional) The {@link Ext.Element#alignTo}
			 *            anchor position to use in aligning to the element
			 *            (defaults to this.defaultAlign)
			 * @param {Ext.ux.StartMenu}
			 *            parentMenu (optional) This menu's parent menu, if
			 *            applicable (defaults to undefined)
			 */
			...
			addTool : function() {
				...
				for (var i = 0; i < l; i++) {
					...
					} else if (typeof el == "object") { // must be menu item
						// config?
			...
			addToolMenuItem : function(config) {
				if (!(config instanceof Ext.menu.Item)) {
					if (typeof config.checked == "boolean") { // must be check
						// menu item
						// config?
