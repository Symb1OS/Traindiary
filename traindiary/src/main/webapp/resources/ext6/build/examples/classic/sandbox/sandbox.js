Ext4.onReady(function() {
	var memoryArray, processArray, colors, memoryStore, processesMemoryStore, cpuLoadStore, data, memoryPieChartConfig, barChartConfig, cpuLoadChartConfig, cpuLoadChartConfig2, win, cpuLoadTimer, pass;

	memoryArray = ['Wired', 'Active', 'Inactive', 'Free'];
	processArray = ['explorer', 'monitor', 'charts', 'desktop', 'Ext3', 'Ext4'];
	colors = ['rgb(244, 16, 0)', 'rgb(248, 130, 1)', 'rgb(0, 7, 255)',
			'rgb(84, 254, 0)'];

	Ext4.chart.theme.Memory = Ext4.extend(Ext4.chart.theme.Base, {
				constructor : function(config) {
					Ext4.chart.theme.Base.prototype.constructor.call(this, Ext4
									.apply({
												colors : colors
											}, config));
				}
	...
	function generateData(a) {
		var data = [], i, names = a, rest = a.length, total = rest, consume;
	...
	function generateCpuLoad() {
		function generate(factor) {
			var value = factor + ((Math.floor(Math.random() * 2) % 2) ? -1 : 1)
	...
	barChartConfig = {
		...
		series : [{
			...
			renderer : function(sprite, record, attr, index, store) {
				var highColor = Ext4.draw.Color.fromString('#e84b67'), lowColor = Ext4.draw.Color
						.fromString('#b1da5a'), value = record.get('memory'), color;
	...
	win = Ext4.widget('window', {
				...
				items : [{
					...
					items : [{
								...
								items : [memoryPieChartConfig, barChartConfig]
							}]
				}]
			});
		...
		cpuLoadTimer = setTimeout(function() {
					...
					if (pass % 5 === 0) {
						processesMemoryStore
								.loadData(generateData(processArray));
					}
