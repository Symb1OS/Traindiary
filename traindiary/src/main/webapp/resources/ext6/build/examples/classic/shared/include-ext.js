 ...
 * and "rtl" url parameters. It first searches for these parameters on the page
 * url, and if they are not found there, it looks for them on the script tag src
 * query string. For example, to include the neptune flavor of ext from an index
 * page in a subdirectory of extjs/examples/: <script type="text/javascript"
...
(function() {
	...
	var scriptEls = document.getElementsByTagName('script'), scriptPath = scriptEls[scriptEls.length
			- 1].src, rtl = getQueryParam('rtl'), themeName = getQueryParam('theme')
			|| 'triton', includeCSS = !hasOption('nocss', scriptPath), useDebug = hasOption('debug'), hasOverrides = !hasOption(
			...
			&& !!{
				...
				crisp : 1,
				'crisp-touch' : 1
			}[themeName], i = 4, devMode = Ext.devMode, extDir = scriptPath, rtlSuffix = (rtl
			? '-rtl'
			: ''), debugSuffix = (devMode ? '-debug' : ''), cssSuffix = rtlSuffix
			+ debugSuffix + '.css', themePackageDir, chartsJS, uxJS, themeOverrideJS, extPrefix, extPackagesRoot;
	...
	if (hasOverrides) {
		...
		// normally this can be done using the defer attribute on the script
		// tag, however
		// this method does not work in IE when in repoDevMode. It seems the
		// reason for
		// this is because in repoDevMode ext-all.js is simply a script that
		// loads other
		// scripts and so Ext is still undefined when the neptune overrides are
		// executed.
		...
		if (devMode) {
			...
			loadScript(themeOverrideJS, true);
			// ux and charts js are not needed in dev mode because they are
