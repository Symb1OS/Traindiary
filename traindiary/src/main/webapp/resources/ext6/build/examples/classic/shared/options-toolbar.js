(function() {
	...
	var scriptTags = document.getElementsByTagName('script'), defaultTheme = 'triton', defaultRtl = false, i = scriptTags.length, requires = [
			'Ext.window.MessageBox', 'Ext.toolbar.Toolbar',
			'Ext.form.field.ComboBox', 'Ext.form.FieldContainer',
		...
		if (Ext.supports.TouchEvents) {
			Ext.getDoc().on({
						touchmove : function(e) {
							// If within a scroller, don't let the document use
							// it
		...
		setTimeout(function() {
			toolbar = Ext.widget({
						...
						items : [{
									...
									/**
									 * Only visible in repoDevMode and on QA
									 * sites
									 */
									xtype : 'button',
									hidden : !(Ext.repoDevMode || location.href
											.indexOf('qa.sencha.com') !== -1),
									enableToggle : true,
									pressed : rtl,
									text : 'RTL',
									margin : '0 5 0 0',
						...
						// Extra constraint margins within default constrain
						// region of parentNode
						constraintInsets : '0 -'
								+ (Ext.getScrollbarSize().width + 5) + ' 0 0'
					});
			toolbar.show();
			toolbar.anchorTo(document.body, Ext.optionsToolbarAlign || 'tr-tr',
					[-(Ext.getScrollbarSize().width + 5), 0], // adjust for
					// scrollbar
					// offsets
					false, // anim
					true // monitor scroll
