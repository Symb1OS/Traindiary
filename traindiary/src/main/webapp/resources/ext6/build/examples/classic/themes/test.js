...
/**
 * @class Ext.ux.event.Driver This is the base class for {@link Recorder} and
 *        {@link Player}.
 */
Ext.define('Ext.ux.event.Driver', {
			...
			/**
			 * @event start Fires when this object is started.
			 * @param {Ext.ux.event.Driver}
			 *            this
			 */

			/**
			 * @event stop Fires when this object is stopped.
			 * @param {Ext.ux.event.Driver}
			 *            this
			 */
			...
			/**
			 * Starts this object. If this object is already started, nothing
			 * happens.
			 */
			start : function() {
				var me = this;
			...
			/**
			 * Stops this object. If this object is not started, nothing
			 * happens.
			 */
			stop : function() {
				var me = this;
 ...
 * 
 * This class manages the playback of an array of "event descriptors". For
 * details on the contents of an "event descriptor", see {@link Recorder}. The
 * events recorded by the {@link Recorder} class are designed to serve as input
 * for this class.
 * 
 * The simplest use of this class is to instantiate it with an
 * {@link #eventQueue} and call {@link #start}. Like so:
 * 
 * var player = Ext.create('Ext.ux.event.Player', { eventQueue: [ ... ], speed:
 * 2, // play at 2x speed listeners: { stop: function () { player = null; // all
 * done } } });
 * 
 * player.start();
 * 
 * A more complex use would be to incorporate keyframe generation after playing
 * certain events.
 * 
 * var player = Ext.create('Ext.ux.event.Player', { eventQueue: [ ... ],
 * keyFrameEvents: { click: true }, listeners: { stop: function () { // play has
 * completed... probably time for another keyframe... player = null; },
 * keyframe: onKeyFrame } });
 * 
 * player.start();
 * 
 * If a keyframe can be handled immediately (synchronously), the listener would
 * be:
 * 
 * function onKeyFrame () { handleKeyFrame(); }
 * 
 * If the keyframe event is always handled asynchronously, then the event
 * listener is only a bit more:
 * 
 * function onKeyFrame (p, eventDescriptor) { eventDescriptor.defer(); // pause
 * event playback...
 * 
 * handleKeyFrame(function () { eventDescriptor.finish(); // ...resume event
 * playback }); }
 * 
 * Finally, if the keyframe could be either handled synchronously or
 * asynchronously (perhaps differently by browser), a slightly more complex
 * listener is required.
 * 
 * function onKeyFrame (p, eventDescriptor) { var async;
 * 
 * handleKeyFrame(function () { // either this callback is being called
 * immediately by handleKeyFrame (in // which case async is undefined) or it is
 * being called later (in which case // async will be true).
 * 
 * if (async) { eventDescriptor.finish(); } else { async = false; } }); //
 * either the callback was called (and async is now false) or it was not //
 * called (and async remains undefined).
 * 
 * if (async !== false) { eventDescriptor.defer(); async = true; // let the
 * callback know that we have gone async } }
 */
Ext.define('Ext.ux.event.Player', {
	extend : 'Ext.ux.event.Driver',

	/**
	 * @cfg {Array} eventQueue The event queue to playback. This must be
	 *      provided before the {@link #start} method is called.
	 */

	/**
	 * @cfg {Object} keyFrameEvents An object that describes the events that
	 *      should generate keyframe events. For example, `{ click: true }`
	 *      would generate keyframe events after each `click` event.
	 */
	keyFrameEvents : {
		click : true
	},
	...
	/**
	 * @cfg {Boolean} pauseForAnimations True to pause event playback during
	 *      animations, false to ignore animations. Default is true.
	 */
	pauseForAnimations : true,

	/**
	 * @cfg {Number} speed The playback speed multiplier. Default is 1.0 (to
	 *      playback at the recorded speed). A value of 2 would playback at 2x
	 *      speed.
	 */
	speed : 1.0,
	...
	/**
	 * @event beforeplay Fires before an event is played.
	 * @param {Ext.ux.event.Player}
	 *            this
	 * @param {Object}
	 *            eventDescriptor The event descriptor about to be played.
	 */

	/**
	 * @event keyframe Fires when this player reaches a keyframe. Typically,
	 *        this is after events like `click` are injected and any resulting
	 *        animations have been completed.
	 * @param {Ext.ux.event.Player}
	 *            this
	 * @param {Object}
	 *            eventDescriptor The keyframe event descriptor.
	 */
	 ...
	 * 
	 * @param {String}
	 *            xpath The XPath-like description of the element.
	 * @return {HTMLElement}
	 */
	getElementFromXPath : function(xpath) {
		...
		el = (parts[0] == '~') ? el.body : el.getElementById(parts[0]
				.substring(1)); // remove '#'
	...
	/**
	 * This method is called after an event has been played to prepare for the
	 * next event.
	 * 
	 * @param {Object}
	 *            eventDescriptor The descriptor of the event just played.
	 */
	...
	/**
	 * This method returns the event descriptor at the front of the queue. This
	 * does not dequeue the event. Repeated calls return the same object (until
	 * {@link #nextEvent} is called).
	 */
	peekEvent : function() {
		var me = this, queue = me.eventQueue, index = me.queueIndex, eventDescriptor = queue[index], type = eventDescriptor
				&& eventDescriptor.type, tmp;

		if (type == 'mduclick') {
			tmp = [	Ext.applyIf({
								type : 'mousedown'
							}, eventDescriptor), Ext.applyIf({
								type : 'mouseup'
							}, eventDescriptor), Ext.applyIf({
								type : 'click'
							}, eventDescriptor)];
	...
	// },
	/**
	 * This method dequeues and injects events until it has arrived at the time
	 * index. If no events are ready (based on the time index), this method does
	 * nothing.
	 * 
	 * @return {Boolean} True if there is more to do; false if not (at least for
	 *         now).
	 */
	processEvents : function() {
		var me = this, animations = me.pauseForAnimations
				&& me.attachTo.Ext.fx.Manager.items, eventDescriptor;
	...
	/**
	 * This method is called when a keyframe is reached. This will fire the
	 * keyframe event. If the keyframe has been handled, true is returned.
	 * Otherwise, false is returned.
	 * 
	 * @param {Object}
	 *            The event descriptor of the keyframe.
	...
	translateEvent : function(eventDescriptor, target) {
		var me = this, event = me.eventObject, modKeys = eventDescriptor.modKeys
				|| '', xy;
	...
	getTarget : function(eventDescriptor) {
		...
		if (eventDescriptor.cmpQuery) {
			me.findTarget(eventDescriptor, Ext.ComponentQuery
							.query(eventDescriptor.cmpQuery)[0]);
		} else {
	...
	findTarget : function(eventDescriptor, cmp) {
		var me = this, x, y, el, offsetX, offsetY;
		if (cmp) {
...
Ext.define('Ext.ux.event.Maker', {
	...
	constructor : function(config) {
		...
		Ext.Array.each(config, function(item) {
					item.el = item.el || 'el';
					if (item.cmpQuery) {
						Ext.Array.each(Ext.ComponentQuery.query(item.cmpQuery),
								function(cmp) {
	...
	__x.player = Ext.create('Ext.ux.event.Player', {
		eventQueue : [{
			...
			screenshot : true
				...
				// },{
				// cmpQuery: 'panel[title=Basic Panel With Toolbars] >
				// toolbar[dock=bottom] > button[text=Menu Button]',
				//                
				...
				// },{
				// cmpQuery: 'panel[title=Basic Panel With Toolbars] >
				// toolbar[dock=bottom] > button[text=Split Button]',
				//                
				// domQuery: '.x-btn-split',
				//                
				// type: 'mduclick',
				...
				// },{
				// cmpQuery: 'panel[title=Basic Panel With Toolbars] >
				// toolbar[dock=bottom] > button[text=Toggle Button]',
				//                
				// type: 'mduclick',
				//                 
				// screenshot: true
				...
				// }, {
				// cmpQuery: 'grid[title=Array Grid] > headercontainer >
				// gridcolumn[text=Company]',
				//                
