...
if (hasOption('diag')) {
	Ext.require(['Ext.diag.layout.ContextItem', 'Ext.diag.layout.Context']);
}
...
function getPanelWithToolbars() {
	...
	return applySizing('panelWithToolbars', {
				...
				bbar : {
					...
					items : ['Toolbar', ' ', '-', isTouchTheme ? null : {
								text : 'Button'
							}, {
...
function getBorderLayout() {
	...
	return applySizing('borderLayout', {
				...
				layout : {
					type : 'border',
					padding : padding[Ext.themeName] || 8 // match padding to
					// theme splitter
					// size
				},
				collapsible : true,
...
function getGrid() {
	var store = getStore(), pagingBar = Ext.widget('pagingtoolbar', {
				store : store,
				displayInfo : true,
	...
	return applySizing('grid', {
				...
				columns : [{
							...
							header : "Change",
							width : isTouchTheme ? 90 : 80,
							sortable : true,
							dataIndex : 'change'
						}, {
							header : "% Change",
							width : isTouchTheme ? 105 : 95,
							sortable : true,
							dataIndex : 'pctChange'
						}, {
							header : "Last Updated",
							width : isTouchTheme ? 130 : 110,
							sortable : true,
							xtype : 'datecolumn',
...
function applySizing(id, config) {
	...
	var info = getSizing(id), keys = ['width', 'height', 'x', 'y'], len = keys.length, i = 0, val;
...
function doThemes() {
	var time = Ext.perf.getTimestamp(), maskedPanel;

	var items = [getBasicPanel(), getCollapsedPanel(),
			maskedPanel = getMaskedPanel(), getFramedPanel(),
			getCollapsedFramedPanel(), getBasicWindow(),
			getPanelWithToolbars(), getFormWidgets(), getBorderLayout(),
			getGrid(), getAccordion(), getScrollingTabs(), getPlainTabs(),
			getDatePicker(), getProgressBar(), getFramedGrid(), getResizer(), 0 // end
	// of
	// list
	// (makes
	// commenting
	...
	mainContainer = Ext.create('Ext.container.Container', applySizing(
					'mainContainer', {
						id : 'main-container',
...
Ext.onReady(function() {
	isTouchTheme = Ext.themeName === 'neptune-touch'
			|| Ext.themeName === 'crisp-touch';
	if (!hasOption('perf')) {
		if (hasOption('delay')) {
