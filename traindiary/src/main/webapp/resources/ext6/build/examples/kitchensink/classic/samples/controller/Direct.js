/**
 * This Controller handles initialization requests for the Ext Direct API shared
 * by different examples; in itself it is an example of dynamic API
 * manipulation.
 * 
 * The primary reason to have a global Controller handling Ext Direct is to keep
 * Direct configuration centralized for the application, as well as avoid
 * binding transient ViewControlles to Provider instances unless necessary.
 */

Ext.define('KitchenSink.controller.Direct', {
	extend : 'Ext.app.Controller',

	requires : ['Ext.direct.RemotingProvider', 'Ext.direct.Manager',
			'Ext.app.domain.Direct'],

	config : {
		/**
		 * @cfg {String} apiUrl Default URL to use for Ext.Direct service
		 *      discovery requests.
		 */
		apiUrl : 'data/direct/api.php',

		/**
		 * @cfg {String} [varName=Ext.app.REMOTING_API] Default variable name to
		 *      use for Ext.Direct API declaration.
		 */
	...
	destroy : function() {
		var providers = this.providers, url;
	...
	/**
	 * Request remote API from the server, and create a new Direct provider when
	 * API declaration is received; alternatively create a new provider if the
	 * caller provided a configuration blob for it.
	 * 
	 * @param {Object}
	 *            options Options to configure Provider request, and to return
	 *            to the caller in synchronous fashion.
	 */
	onDirectConnect : function(options) {
		var me = this, providerCfg = options.providerCfg, url, provider, request;

		url = (providerCfg && providerCfg.url) || options.apiUrl
				|| me.getApiUrl();
	...
	/**
	 * Request to disconnect a Direct provider. This event is fired by
	 * ViewControllers being destroyed, to notify Direct controller that they no
	 * longer need the specified provider.
	 * 
	 * @param {String}
	 *            url Service URL for a provider
	 */
