Ext.define('KitchenSink.data.FileTree', {
	requires : ['KitchenSink.data.Init', 'Ext.ux.ajax.XmlSimlet'],

	/*
	 * This script produced the data below:
	 * 
	 * var file = new java.io.File('src'); console.log('Listing ' +
	 * file.getAbsolutePath());
	 * 
	 * function listDir(dir) { var files = dir.listFiles(), ret = [], i = 0,
	 * length = files.length, months = ['Jan', 'Feb', 'Mar', 'Apr', 'May',
	 * 'Jun', 'July', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], file, data, modified,
	 * hour, modStr, size;
	 * 
	 * for (; i < length; ++i) { file = files[i]; data = { text : file.getName() + '' };
	 * modified = new Date(file.lastModified()); hour = modified.getHours();
	 * modStr = months[modified.getMonth()] + ' ' + modified.getDate() + ', ' +
	 * modified.getFullYear() + ', ' + (hour < 12 ? hour : (hour - 12)) + ':' +
	 * modified.getMinutes() + ' ' + (hour < 12 ? 'am' : 'pm');
	 * 
	 * if (file.isDirectory()) { data.qtip = 'Type: Folder<br />Last Modified: ' +
	 * modStr; data.children = listDir(file); } else { size = (file.length() /
	 * 1024).toFixed(2);
	 * 
	 * data.qtip = 'Type: JavaScript File<br />Last Modified: ' + modStr + '<br />Size: ' +
	 * size + ' KB'; }
	 * 
	 * ret.push(data); }
	 * 
	 * return ret; }
	 * 
	 * var obj = listDir(file); console.log(JSON.stringify(obj));
	 * 
	 * Steps: 1) Save to ./ext/dump.js 2) sencha js dump.js > tree.json 3)
	 * Copy/pretty print/paste
	 */
	tree : [{
		text : 'src',
		children : [{
