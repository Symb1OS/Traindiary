Ext.define('KitchenSink.model.grid.Employee', {
			...
			fields : [{
						...
						convert : function(v, rec) {
							return rec.editing ? v : rec.get('forename') + ' '
									+ rec.get('surname');
						}
					}, {
						name : 'email'
			...
			set : function(name, value) {
				...
				// "name" is a calculated field, so update it on edit of
				// "forename" or "surname".
				if (data.forename || data.surname) {
					data.name = (data.forename || this.get('forename')) + ' '
							+ (data.surname || this.get('surname'));
				}
