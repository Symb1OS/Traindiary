Ext.define('KitchenSink.store.Plot', {
	...
	fn : [function(x) {
			...
			// TODO: If exceeded, we should consider that the plotted function
			// is a non-continuous one
			// TODO: (even if there are no gaps in the data) and has a jump at
			// that increment.
			// TODO: The problem with this is that it's hard to pick a value
			// that works well for all functions.
	...
	traverseFunctions : function() {
		var delta = arguments[0], l = arguments.length, data = [], i, j, y, rec;
		for (i = -2; i <= 2; i += delta) {
			rec = {
				x : i
			};
	...
	generateData : function() {
		var me = this;
		return me.traverseFunctions(me.xStep,
				me.fn[me.fnIndex++ % me.fn.length]);
	},
