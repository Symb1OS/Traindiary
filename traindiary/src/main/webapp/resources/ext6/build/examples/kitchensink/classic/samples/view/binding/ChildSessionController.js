...
Ext.define('KitchenSink.view.binding.ChildSessionController', {
	...
	createDialog : function(record) {
		...
		this.dialog = view.add({
					...
					viewModel : {
						data : {
							title : record
									? 'Edit: ' + record.get('name')
									: 'Add Customer'
						},
						// If we are passed a record, a copy of it will be
						// created in the newly spawned session.
						// Otherwise, create a new phantom customer in the
						// child.
						links : {
							theCustomer : record || {
								type : 'Customer',
					...
					// Creates a child session that will spawn from the current
					// session
					// of this view.
					session : true
				});
	...
	onRemoveCustomerClick : function(button) {
		var customerGrid = this.lookupReference('customerGrid'), selection = customerGrid
	...
	onSaveClick : function() {
		...
		// parent session.
		var dialog = this.dialog, form = this.lookupReference('form'), isEdit = this.isEdit, id;

		if (form.isValid()) {
			if (!isEdit) {
				// Since we're not editing, we have a newly inserted record.
				// Grab the id of
			...
			if (!isEdit) {
				// Use the id of that child record to find the phantom in the
				// parent session,
				// we can then use it to insert the record into our store
				this.getStore('customers').add(this.getSession().getRecord(
						'Customer', id));
			}
			this.onCancelClick();
		}
