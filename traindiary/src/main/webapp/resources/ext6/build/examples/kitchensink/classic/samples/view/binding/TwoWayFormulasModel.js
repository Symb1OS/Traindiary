Ext.define('KitchenSink.view.binding.TwoWayFormulasModel', {
			...
			formulas : {
				age : {
					...
					set : function(age) {
						...
						if (!date) {
							...
							date = new Date(now.getFullYear() - age, date
											.getMonth(), date.getDate());
							if (this.getAge(date) !== age) {
								date = new Date(now.getFullYear() - age - 1,
				...
				fullName : {
					...
					// By providing the set method "fullName" is two-way
					// bindable.
					set : function(value) {
						var space = value.indexOf(' '), split = (space < 0)
								? value.length
								: space;
