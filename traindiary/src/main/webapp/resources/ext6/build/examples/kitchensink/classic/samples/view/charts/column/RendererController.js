Ext.define('KitchenSink.view.charts.column.RendererController', {
	...
	onRefresh : function() {
		var chart = this.lookupReference('chart'), store = chart.getStore();

		store.setData(store.generateData(7));
	},

	onG1SeriesRender : function(sprite, config, rendererData, index) {
		var store = rendererData.store, storeItems = store.getData().items, record = storeItems[index], last = storeItems.length
				- 1, surface = sprite.getSurface(), isRtl = surface
				.getInherited().rtl, changes = {}, lineSprites, firstColumnConfig, firstData, lastData, growth, string;
		if (!record) {
			return;
		}
		...
		if (index == 0 || index == last) {
			...
			if (index == 0) {
				...
				var x1 = firstColumnConfig.x + firstColumnConfig.width, y1 = firstColumnConfig.y, x2 = changes.x, y2 = changes.y;

				growth = Math.round(100 * (lastData - firstData)
						/ (firstData || 1));
				...
				lineSprites[1].setAttributes({
							...
							y : firstColumnConfig.y
									+ (changes.y - firstColumnConfig.y) / 2
									+ 10,
	...
	onG2SeriesRender : function(sprite, config, rendererData, index) {
		var store = rendererData.store, storeItems = store.getData().items, last = storeItems.length
				- 1, record = storeItems[index], diff = record
				&& Math.round(record.data['g2'] - record.data['g1']), changes = {}, surface = sprite
				.getSurface(), isRtl = surface.getInherited().rtl, textSprites, textSprite, rectSprite;
		if (!record) {
			return;
		}
		// This renderer function draws a red label if series #2 is greater than
		// series #1.
		// The label displays the difference between the values of series #1 and
		// series #2.
		//
		// Note: The two renderer functions in this test case cannot be
		// consolidated. The red labels
		// are rendered here because they are positioned relatively to the
		// series #2 columns.
		if (diff > 0) {
			...
			rectSprite.setAttributes({
						x : config.x
								+ (index == last ? -17 : (isRtl ? -50 : 16)),
						y : config.y - 36,
						width : 30
								+ (diff >= 10 ? (diff >= 100 ? (diff >= 1000
										? 18
										: 12) : 6) : 0),
						height : 18,
						stroke : 'tomato',
