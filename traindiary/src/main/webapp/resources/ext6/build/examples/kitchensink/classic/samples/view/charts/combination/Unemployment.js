/**
 * The US Unemployment Infographic shows how to create what might seem like a
 * distinct type of chart, but in fact is only a fancy combination of standard
 * features: a polar chart with multiple 'pie' series and a cartesian chart. The
 * unique look is achieved by a heavy use of sprites and renderers to style the
 * chart, and the only custom component is the 'arctext' sprite that is used to
 * put a text along a curve.
 * 
...
Ext.define('KitchenSink.view.charts.combination.Unemployment', {
	...
	items : [{
		...
		animation : false,
		background : 'white',
		// Custom sprites that are used to decorate the chart to give it the
		// infographic look.
		sprites : [{
					type : 'rect',
					x : 50,
					...
					type : 'path', // The stripe containing the 'SENCHA
									// INFOGRAPHIC' text.
		...
		series : [{
			...
			// To minimize redraw time we perform the highlighting in a separate
			// chart,
			// so the larger portion of the infographic is only rendered once.
			{
				...
				series : [{
							// Translucent dummy series used for highlighting
							// only.
							// Rendered on top of all the series used to show
							// the data.
							type : 'pie',
							rotation : -Math.PI / 60,
							angleField : 'span',
							donut : 57,
							subStyle : {
								fillStyle : 'none', // not visible unless
													// highlighted
								strokeStyle : 'none',
								lineWidth : 1
							},
							// Instead of using the 'highlight' config,
							// which uses the default highlight style of the
							// series
							// if set to 'true', or adds to the default
							// highlight style
							// if given an object, we override the default
							// highlight
							// style completely by providing our own
							// 'highlightCfg'.
							highlightCfg : {
							...
							// highlight: {
							// fillStyle: 'rgba(0,0,0,0.2)',
							// margin: 0 // override the value of default
							// highlight style
							// },
							...
							// where the text "Recession December 2007" is
							// shown.
							...
							angleField : 'dummy',
							donut : 63,
							// Make a dummy store with a single record for this
							// series
							// so that only one pie sector (a donut ring) is
							// created,
							// intead of 50+ sectors (one for each state +
							// separators).
