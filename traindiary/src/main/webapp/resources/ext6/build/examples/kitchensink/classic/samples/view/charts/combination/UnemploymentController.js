Ext.define('KitchenSink.view.charts.combination.UnemploymentController', {
	 ...
	 * @param sprites
	 *            The array to add the created tick sprites to (for future
	 *            reference).
	 */
	addTicks : function(surface, attr, angles, sprites) {
		...
		for (i = 0, ln = angles.length; i < ln; i++) {
			...
			sprites.push(surface.add({
						...
						fromX : attr.centerX + (attr.endRho + linePadding)
								* Math.cos(angle),
						fromY : attr.centerY + (attr.endRho + linePadding)
								* Math.sin(angle),

						toX : attr.centerX
								+ (attr.endRho + linePadding + tickSize)
								* Math.cos(angle),
						toY : attr.centerY
								+ (attr.endRho + linePadding + tickSize)
								* Math.sin(angle)
	...
	onDummySliceRender : function(sprite, config, data, index) {
		...
		var me = this, ln = me.regions.length, record = data.store.getAt(index), label = record
				.get('label'), spriteSurface = sprite.getSurface(), // 'series'
																	// surface
		chart = spriteSurface.ownerCt, overlaySurface = chart
				.getSurface('overlay'), attr = sprite.attr, region;
		...
		if (label !== '' && label !== 'year') {
			...
			if (me.regionIndex !== ln) {
				...
				// we can create region sprites on the first 'swipe' through the
				// sectors
				...
				if (label === region.start) {
					...
					me.addTicks(overlaySurface, attr, [me.startAngle,
									me.endAngle], region.sprites);
					...
					region.sprites.push(overlaySurface.add({
						...
						angle : ((me.startAngle + me.endAngle) * 0.5 + attr.rotationRads)
								/ Math.PI * 180,
						translationX : attr.translationX,
						translationY : attr.translationY,
						template : {
	...
	onSliceRender2012 : function(sprite, config, data, index) {
		// Please see the comments in the KitchenSink.store.Unemployment class
		// for more info on the meaning of the record fields.
		var record = data.store.getAt(index), label = record.get('label'), unemployment = record
				.get('y2012'), style = {};
		...
		} else { // a sector that shows the change in unemployment with a
					// color
	...
	onSliceRender2011 : function(sprite, config, data, index) {
		var record = data.store.getAt(index), label = record.get('label'), style = {};
	...
	onItemHighlight : function(chart, item) {
		...
		// about the highlighted item, like its 'index', the 'series' it belongs
		// to,
		// the associated store 'record' and its 'field'.

		var regions = this.regions, label = item.record.get('label'), cartesianChart = this
				.lookupReference('cartesian'), data = item.record.data, i, j, ik, jk, region, sprite;
		...
		if (!label || label === 'year') {
			...
			// Display unemployment changes for the highlighted state inside the
			// cartesian chart.
		...
		// Find the region the highlighted state sector belongs to and highlight
		// its sprites,
		// while unhighlighting all other regions.
		for (i = 0, ik = regions.length; i < ik; i++) {
			region = regions[i];
			if (item.index >= region.startIndex
					&& item.index <= region.endIndex) {
	...
	onAfterRender : function(chart) {
		...
		var series = chart.getSeries()[0], store = chart.getStore(), index = store
				.find('label', 'CA');
	...
	onBeforeRender : function() {
		// Make Ext.draw.TextMeasurer use the Ext.util.TextMetrics to measure
		// text,
	...
	onDestroy : function() {
		var regions = this.regions, i, region;
