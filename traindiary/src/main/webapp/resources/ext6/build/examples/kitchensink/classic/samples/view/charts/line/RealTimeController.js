Ext.define('KitchenSink.view.charts.line.RealTimeController', {
	...
	onAxisLabelRender : function(axis, label, layoutContext) { // only render
																// interger
																// values
		return Math.abs(layoutContext.renderer(label) % 1) < 1e-5 ? Math
				.round(label) : '';
	},
	...
	addNewTimeData : function() {
		var me = this, chart = me.lookupReference('time-chart'), store = chart
				.getStore(), count = store.getCount(), xAxis = chart.getAxes()[1], visibleRange = 10000, second = 1000, xValue, lastRecord;
	...
	addNewNumberData : function() {
		var chart = this.lookupReference('number-chart'), store = chart
				.getStore(), count = store.getCount(), xAxis = chart.getAxes()[1], visibleRange = 20, minY = 0, maxY = 100, deltaY = 5, xValue, lastRecord;
		...
		if (count > 0) {
			...
			store.add({
						xValue : xValue,
						yValue : this.getNextValue(lastRecord.get('yValue'),
								minY, maxY, deltaY)
					});
			...
			store.add({
						xValue : 0,
						yValue : this.getNextValue((minY + maxY) / 2, minY,
								maxY)
					});
			chart.animationSuspended = false;
		}
