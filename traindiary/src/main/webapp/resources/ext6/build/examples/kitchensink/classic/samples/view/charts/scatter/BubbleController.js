Ext.define('KitchenSink.view.charts.scatter.BubbleController', {
	...
	onRefresh : function() {
		var me = this, chart = me.lookupReference('chart'), leftAxis = chart
				.getAxes()[0], store = chart.getStore();
	...
	onDropBubble : function() {
		var me = this, chart = me.lookupReference('chart'), store = chart
				.getStore(), leftAxis = chart.getAxes()[0];
	...
	interpolateColor : function(lambda, minSrc, maxSrc) {
		var me = this, fromHSL = me.fromHSL, toHSL = me.toHSL;
	...
	onItemRender : function(sprite, config, rendererData, index) {
		var me = this, store = rendererData.store, storeItem = store.getData().items[index];
	...
	createData : function(count, isZeroed) {
		var me = this, data = [], record = isZeroed ? {
			x : 0,
			g0 : 0,
			g1 : 0,
			g2 : 0,
