Ext.define('KitchenSink.view.direct.FormController', {
			...
			config : {
				/*
				 * Note that we're overriding the default URI here to load the
				 * API that is specific to Form handling and initialize it as a
				 * separate Ext Direct Provider.
				 */
				apiUrl : 'data/direct/api.php?form=true',

				/*
				 * Note that Direct requests will batch together if they occur
				 * within the enableBuffer delay period (in milliseconds). Slow
				 * the buffering down from the default of 10ms to 100ms for this
				 * Provider only.
				 */
				providerCfg : {
					enableBuffer : 100
