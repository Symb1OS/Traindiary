Ext.define('KitchenSink.view.direct.GenericController', {
	...
	requires : ['Ext.direct.PollingProvider'],
	...
	trulyFinishInit : function() {
		var me = this, urls = me.providerUrls, options, provider, view, updateFn;
	...
	setInterval : function(field) {
		var me = this, urls = me.providerUrls, interval, provider;
		...
		if (Ext.isNumeric(interval) && provider) {
			...
			if (interval > 0) {
				...
				me.updateView('Polling interval set to ' + interval
						+ ' seconds');
			} else {
				provider.disconnect();
	...
	onEcho : function(result, event, success, options) {
		...
		content = Ext.String
				.format(
						...
						transaction.action, transaction.method, Ext
								.encode(result));
		...
		if (event.status) {
			...
			content = Ext.String
					.format(
							...
							transaction.action, transaction.method,
							event.message);
		}
