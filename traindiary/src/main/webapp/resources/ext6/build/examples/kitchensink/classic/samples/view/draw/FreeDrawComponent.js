...
Ext.define('KitchenSink.view.FreeDrawComponent', function() {
	...
	return {
		...
		listeners : {
			element : 'element',
			// The type of events that are supported for each framework and
			// platform are...
			...
			//
			// Since this is an ExtJS example, we can listen to Mouse events
			// only.
			//
			// If we wanted the example to run on Sencha Touch also, we would
			// have
			// to listen to both Mouse events and Drag events (listening to
			// Mouse
			...
			mousedown : function(e) {
				var targetElement = this, me = Ext.getCmp(targetElement.id), surface = me
						.getSurface();
				if (!me.sprite) {
					var xy = surface.getEventXY(e), x = xy[0], y = xy[1];
					...
					me.sprite = surface.add({
								type : 'path',
								path : ['M', me.list[0], me.list[1], 'L',
										me.list[0] + 1e-1, me.list[1] + 1e-1],
								lineWidth : 30 * Math.random() + 10,
								lineCap : 'round',
								lineJoin : 'round',
								strokeStyle : new Ext.draw.Color(Math.random()
												* 127 + 128, Math.random()
												* 127 + 128, Math.random()
												* 127 + 128)
							});
			...
			mousemove : function(e) {
				var targetElement = this, me = Ext.getCmp(targetElement.id), surface = me
						.getSurface();
				if (me.sprite) {
					var xy = surface.getEventXY(e), x = xy[0], y = xy[1], dx = me.lastEventX
							- x, dy = me.lastEventY - y, D = 10;
		...
		onMouseUp : function(e) {
			var targetElement = this, me = Ext.getCmp(targetElement.id);
			me.sprite = null;
		},
