...
Ext.define('KitchenSink.view.draw.bounce.BounceController', {
			...
			onAfterRender : function() {
				var me = this, draw = me.lookupReference('draw'), surface = draw
						.getSurface(), logo = surface.get('logo');
			...
			onRender : function() {
				var me = this, rect = me.surface.getRect(), bbox = me.logo
						.getBBox(true), bounced = false, p, ghost;

				// Update current position based on velocity and acceleration.
				me.position = p = me.position.add(me.velocity)
						.add(me.acceleration);
				...
				if (bounced) {
					...
					// The scale and opacity of the ghost will be updated on
					// every frame
					...
					// Remove the sprite from the surface when the animation is
					// done.
					ghost.fx.on('animationend', function() {
								ghost.remove();
							});
				} else {
					// Decrease the logo's acceleration on every move after a
					// bounce.
				...
				// The logo sprite does not have its animation modifier
				// configured
