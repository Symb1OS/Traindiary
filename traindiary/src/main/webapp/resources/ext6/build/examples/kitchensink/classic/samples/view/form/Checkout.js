/**
 * This shows an example of a common shopping cart checkout form. It
 * demonstrates uses of FieldContainer and various layouts for arranging and
 * aligning fields, ComboBox fields for state and month selection, and listening
 * to change events to automatically copy values from Mailing Address to Billing
 * Address fields.
 */
Ext.define('KitchenSink.view.form.Checkout', {
		...
		if (!this.monthStore) {
			this.self.prototype.monthStore = new Ext.data.Store({
						...
						data : (function() {
							...
							Ext.Array.forEach(Ext.Date.monthNames, function(
											name, i) {
										data[i] = {
											name : name,
											num : i + 1
	...
	onMailingAddrFieldChange : function(field) {
		var copyToBilling = this.down('[name=billingSameAsMailing]').getValue(), copyField = this
				.down('[name=' + field.billingFieldName + ']');
	...
	/**
	 * Enables or disables the billing address fields according to whether the
	 * checkbox is checked. In addition to disabling the fields, they are
	 * animated to a low opacity so they don't take up visual attention.
	 */
	onSameAddressChange : function(box, checked) {
		var fieldset = box.ownerCt;
		Ext.Array.forEach(fieldset.previousSibling().query('textfield'),
				this.onMailingAddrFieldChange, this);
