Ext.define('KitchenSink.view.form.CustomErrorHandlingController', {
	...
	submitRegistration : function() {
		 ...
		 * response... form.submit({ clientValidation: true, url:
		 * 'register.php', success: function(form, action) { //... }, failure:
	...
	updateErrorState : function(cmp, state) {
		var me = this, errorCmp = me.lookupReference('formErrorState'), view, form, fields, errors;

		view = me.getView();
		form = view.getForm();

		// If we are called from the form's validitychange event, the state will
		// be false if invalid.
		// If we are called from a field's errorchange event, the state will be
		// the error message.
