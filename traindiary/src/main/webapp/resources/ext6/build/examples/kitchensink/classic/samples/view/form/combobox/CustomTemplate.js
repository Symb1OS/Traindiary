/**
 * This example illustrates a combo box which loads data from a local array and
 * uses a custom item template.
 */
Ext.define('KitchenSink.view.form.combobox.CustomTemplate', {
	...
	items : [{
		...
		items : [{
			...
			queryMode : 'local',
			listConfig : {
				itemTpl : ['<div data-qtip="{state}: {description}">{state} ({abbr})</div>']
			}
		}]
	}]
});
