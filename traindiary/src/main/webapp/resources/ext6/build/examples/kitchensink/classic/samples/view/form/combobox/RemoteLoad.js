/**
 * This example illustrates a combo box which loads the data source from a
 * server but queries the local result set. It uses a custom item template for
 * the dropdown list.
 */
Ext.define('KitchenSink.view.form.combobox.RemoteLoad', {
	...
	// <example>
	requires : ['KitchenSink.model.State', 'KitchenSink.store.RemoteStates'],
	...
	items : [{
		...
		items : [{
			...
			minChars : 0,
			queryMode : 'local',
			tpl : ['<ul class="x-list-plain">', '<tpl for=".">', '<li class="',
					Ext.baseCSSPrefix, 'grid-group-hd ', Ext.baseCSSPrefix,
					'grid-group-title">{abbr}</li>',
					'<li class="x-boundlist-item">', '{state}, {description}',
					'</li>', '</tpl>', '</ul>']
		}]
	}]
});
