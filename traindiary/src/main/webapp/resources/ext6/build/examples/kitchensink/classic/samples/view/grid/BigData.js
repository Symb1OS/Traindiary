 ...
 * without overloading the DOM. It also uses locking columns, and incorporates
 * the GroupSummary feature. Filtering is enabled on certain columns using the
 * FilterFeature.
 * 
 * As an illustration of the ability of grid columns to act as containers, the
 * Title column has a filter text field built in which filters as you type.
 * 
 * The grid is editable using the RowEditing plugin.
 * 
 * The `multiColumnSort` config is used to allow multiple columns to have
 * sorters.
 * 
 * It is also possible to export the grid data to Excel. This feature is
 * available in Ext JS Premium.
 */
Ext.define('KitchenSink.view.grid.BigData', {
	extend : 'Ext.grid.Panel',
	requires : ['Ext.grid.filters.Filters', 'Ext.grid.plugin.Exporter'],
	xtype : 'big-data-grid',
	store : 'BigData',
	columnLines : true,
	height : 400,
	...
	columns : [{
				...
				items : {
					xtype : 'textfield',
					reference : 'nameFilterField', // So that the Controller
				...
				editor : {
					...
					initComponent : function() {
						this.store = this.column.up('tablepanel').store
								.collect(this.column.dataIndex, false, true);
