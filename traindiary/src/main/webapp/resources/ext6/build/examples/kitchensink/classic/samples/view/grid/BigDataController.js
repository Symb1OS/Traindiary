 ...
 * 
 * Provides logic which is referenced by listeners, handlers and renderers in
 * the view which are configured as strings. They are resolved to members of
 * this class.
 * 
 */
Ext.define('KitchenSink.view.grid.BigDataController', {
	...
	// Used as an editRenderer by BigData to display an uneditable field in the
	// RowEditor
	bold : function(v) {
		return "<b>" + v + "</b>";
	},
	...
	onNameFilterKeyup : function() {
		...
		filterField = this.lookupReference('nameFilterField'), filters = grid.store
				.getFilters();
	...
	init : function() {
		...
		if (!Ext.supports.Touch) {
			// Plugins are instantiated at this time, we must add an
			// instantiated Plugin, not a config
	...
	// This method is called as a listener to the grid's headermenucreated
	// event.
