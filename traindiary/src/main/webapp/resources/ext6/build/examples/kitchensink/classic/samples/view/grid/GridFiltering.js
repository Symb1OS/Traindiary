...
Ext.define('KitchenSink.view.grid.GridFiltering', {
			...
			// Need a minHeight. Neptune resizable framed panels are
			// overflow:visible
			// so as to enable resizing handles to be embedded in the border
			// lines.
			minHeight : 200,
			// </example>
			...
			columns : [{
						...
						// Specify that this column has an associated Filter.
						// This is
						// processed by the gridfilters plugin. If this is a
						// string,
						...
						// As an object, the type property indicates the type of
						// filter to
						// apply. All other properties configure that filter
						// instance.
						filter : {
							type : 'string',
							itemDefaults : {
						...
						filter : 'list' // Use the unique field values for the
										// pick list
					}, {
						xtype : 'datecolumn',
						dataIndex : 'date',
						...
						filter : true
						// use dataIndex first then fallback to column type
				}	, {
						dataIndex : 'visible',
						text : 'Visible',
						width : 80,
			...
			onClearFilters : function() {
				// The "filters" property is added to the grid (this) by
				// gridfilters
