...
Ext.define('KitchenSink.view.grid.GroupedGrid', {
	...
	// Need a minHeight. Neptune resizable framed panels are overflow:visible so
	// as to
	...
	initComponent : function() {
		...
		var store = this.getStore(), toggleMenu = [];
	...
	onGroupChange : function(store, grouper) {
		var grouped = store.isGrouped(), groupBy = grouper ? grouper
				.getProperty() : '', toggleMenuItems, len, i = 0;
		...
		if (grouped && groupBy === 'cuisine') {
			...
			for (len = toggleMenuItems.length; i < len; i++) {
				toggleMenuItems[i].setChecked(this.groupingFeature
								.isExpanded(toggleMenuItems[i].text));
			}
	...
	onGroupCollapse : function(v, n, groupName) {
		if (!this.down('[text=Toggle groups...]').disabled) {
			this.down('menucheckitem[text=' + groupName + ']').setChecked(
					false, true);
		}
	},
	...
	onGroupExpand : function(v, n, groupName) {
		if (!this.down('[text=Toggle groups...]').disabled) {
			this.down('menucheckitem[text=' + groupName + ']').setChecked(true,
					true);
		}
	}
});
