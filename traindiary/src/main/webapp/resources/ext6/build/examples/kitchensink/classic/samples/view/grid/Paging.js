...
Ext.define('KitchenSink.view.grid.Paging', {
	...
	requires : ['Ext.data.*', 'Ext.grid.*', 'Ext.util.*', 'Ext.toolbar.Paging',
			'Ext.ux.PreviewPlugin', 'KitchenSink.model.grid.ForumThread'],
	xtype : 'paging-grid',
	...
	initComponent : function() {
		...
		Ext.apply(this, {
					...
					columns : [{
								...
								// TODO: This poses an issue in subclasses of
								// Grid now because Headers are now Components
								// therefore the id will be registered in the
								// ComponentManager and conflict. Need a way to
								// add additional CSS classes to the rendered
								// cells.
								id : 'topic',
								text : "Topic",
								dataIndex : 'title',
					...
					bbar : Ext.create('Ext.PagingToolbar', {
								...
								items : ['-', {
									...
									toggleHandler : function(btn, pressed) {
										btn.up('grid').getPlugin('preview')
												.toggleExpanded(pressed);
										btn.setText(pressed
												? 'Hide Preview'
												: 'Show Preview');
									}
	...
	renderLast : function(value, p, model) {
		return Ext.String.format('{0}<br/>by {1}', Ext.Date.dateFormat(value,
						'M j, Y, g:i a'), model.get('lastposter'));
	}
});
