 ...
 * dynamically. By default, we start with no store or columns, we can define
 * them later using the reconfigure method.
 */
Ext.define('KitchenSink.view.grid.Reconfigure', {
	extend : 'Ext.container.Container',

	requires : ['Ext.grid.*', 'Ext.layout.container.HBox',
			'Ext.layout.container.VBox', 'KitchenSink.model.grid.Office',
	...
	lastNames : ['Jones', 'Smith', 'Lee', 'Wilson', 'Black', 'Williams',
			'Lewis', 'Johnson', 'Foot', 'Little', 'Vee', 'Train', 'Hot', 'Mutt'],
	firstNames : ['Fred', 'Julie', 'Bill', 'Ted', 'Jack', 'John', 'Mark',
			'Mike', 'Chris', 'Bob', 'Travis', 'Kelly', 'Sara'],
	...
	onShowEmployeesClick : function() {
		...
		grid.reconfigure(this.createEmployeeStore(), [{
							...
							width : this.profileInfo.employeeWidth,
							text : 'Employee No.',
	...
	createOfficeStore : function() {
		var data = [], i = 0, usedNames = {}, usedCities = {};
	...
	generateName : function() {
		var lasts = this.lastNames, firsts = this.firstNames, lastLen = lasts.length, firstLen = firsts.length, getRandomInt = Ext.Number.randomInt, first = firsts[getRandomInt(
				0, firstLen - 1)], last = lasts[getRandomInt(0, lastLen - 1)];
