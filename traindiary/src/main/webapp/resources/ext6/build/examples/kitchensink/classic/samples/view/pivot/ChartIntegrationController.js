...
Ext.define('KitchenSink.view.pivot.ChartIntegrationController', {
	...
	onPivotDone : function() {
		var me = this, view = me.getView(), pivot = view.down('pivotgrid'), chart = view
				.down('chart');
	...
	chartRenderer : function(v) {
		var matrix = this.getMatrix(), item = matrix.leftAxis.findTreeElement(
				"key", v);

		return item ? item.node.name : (v == matrix.grandTotalKey
				? matrix.textGrandTotalTpl
				: v);
	},

	getTitles : function(pivot) {
		var data = [], cols = pivot.getColumns(), len = cols.length, i;
