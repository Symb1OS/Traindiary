...
Ext.define('KitchenSink.view.pivot.RemoteCalculationsController', {
	...
	onPivotDone : function() {
		var me = this, preview = Ext.ComponentQuery.query('#codePreview')[0], viewData = preview
				.down('#codeRemoteData'), viewParams = preview
				.down('#codeRemoteParams'), itemData = {
			xtype : 'codeContent',
	...
	getJsonResponse : function() {
		...
		text += ' */\n\n';
		text += JSON.stringify(Ext.ux.ajax.SimManager.getSimlet(this.getView()
						.getMatrix().url).lastResponse, null, 4);
