/**
 * This example shows how filters can be applied to the TreeStore of a Tree.
 * TreeStore filters are added in the same way as other store filters.
 * 
 * The filter function is called on child nodes first so that it can use child
 * node state when calculating a parent node\'s filter result.
 * 
	...
	tbar : [{
				...
				try {
					...
					tree.store.filter({
						filterFn : function(node) {
							...
							// Visibility of leaf nodes is whether they pass the
							// test.
							// Visibility of branch nodes depends on them having
							// visible children.
							visible = node.isLeaf()
									? v.test(node.get('title'))
									: false, i;

							// We're visible if one of our child nodes is
							// visible.
							// No loop body here. We are looping only while the
							// visible flag remains false.
							// Child nodes are filtered before parents, so we
							// can check them here.
							// As soon as we find a visible child, this branch
		...
		listeners : {
			beforerender : function() {
				var me = this, tree = me.up('treepanel'), root = tree
						.getRootNode(), leafCount = 0;
