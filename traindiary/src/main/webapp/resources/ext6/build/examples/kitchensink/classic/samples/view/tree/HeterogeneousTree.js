/**
 * This examples shows how heterogeneous models can be read into a TreeStore
 * offering the ability to have different entity types at different levels of a
 * tree.
 * 
 * The Model classes at each level of the tree are different types. So you can
 * use associated but different data types in your trees. The key to this is the
 * `typeProperty` config on the Proxy's Reader.
 * 
 * The toolbar is aware of the type of the selected node and knows what kind of
 * new entity to add.
 */
Ext.define('KitchenSink.view.tree.HeterogeneousTree', {
	...
	bbar : [{
				...
				listeners : {
					keydown : function(inputField, e) {
						...
						} else if (e.keyCode === Ext.EventObject.TAB
								&& e.shiftKey) {
							e.stopEvent();
							panel.view
									.focusRow(panel.selModel.getSelection()[0]
											|| 0);
						}
					}
				}
	...
	addClick : function() {
		var target = this.selModel.getSelection()[0] || this.getRootNode(), inputField = this
				.down('#new-name'), value = inputField && inputField.getValue(), store = this
				.getStore(), node;
