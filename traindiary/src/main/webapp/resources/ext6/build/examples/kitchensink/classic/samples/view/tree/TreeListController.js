Ext.define('KitchenSink.view.tree.TreeListController', {
			...
			onToggleMicro : function(button, pressed) {
				var treelist = this.lookupReference('treelist'), navBtn = this
						.lookupReference('navBtn'), ct = treelist.ownerCt;
				...
				// IE8 has an odd bug with handling font icons in pseudo
				// elements;
				// it will render the icon once and not update it when something
				// like text color is changed via style addition or removal.
				// We have to force icon repaint by adding a style with forced
				// empty
				// pseudo element content, (x-sync-repaint) and removing it back
				// to work
				// around this issue.
				// See this:
