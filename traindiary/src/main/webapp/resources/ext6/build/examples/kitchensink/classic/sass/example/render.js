/*
 * This file is generated by Sencha Cmd and should NOT be edited. It will be
...
Ext.theme = {
	/**
	 * The array of all component manifests. These objects have the following
	 * set of properties recognized by the slicer:
	 * 
	 * @private
	 */
	_manifest : [],
	...
	/**
	 * The collection of shortcuts for a given alias (e.g., 'widget.panel').
	 * This is an object keyed by alias whose values are arrays of shortcut
	 * definitions.
	 * 
	 * @private
	 */
	...
	/**
	 * Adds one ore more component entries to the theme manifest. These entries
	 * will be instantiated by the `Ext.theme.render` method when the page is
	 * ready.
	 * 
	 * Usage:
	 * 
	 * Ext.theme.addManifest({ xtype: 'widget.menu', folder: 'menu', delegate:
	 * '.x-menu-item-link', filename: 'menu-item-active', config: { floating:
	 * false, width: 200, items: [{ text: 'test', cls: 'x-menu-item-active' }] }
	 * },{ //... });
	 * 
	 * @param manifest
	 *            {Object} An object with type of component, slicing information
	 *            and component configuration. If this parameter is an array,
	 *            each element is treated as a manifest entry. Otherwise, each
	 *            argument passed is treated as a manifest entry.
	 * 
	 * @param manifest.xtype
	 *            {String} The xtype ('grid') or alias ('widget.grid'). This is
	 *            used to specify the type of component to create as well as a
	 *            potential key to any `shortcuts` defined for the xtype.
	 * 
	 * @param manifest.config
	 *            {Object} The component configuration object. The properties of
	 *            this depend on the `xtype` of the component.
	 * 
	 * @param [manifest.delegate]
	 *            {String} The DOM query to use to select the element to slice.
	 *            The default is to slice the primary element of the component.
	 * 
	 * @param [manifest.parentCls]
	 *            An optional CSS class to add to the parent of the component.
	 * 
	 * @param [manifest.setup]
	 *            {Function} An optional function to be called to initialize the
	 *            component.
	 * @param manifest.setup.component
	 *            {Ext.Component} The component instance
	 * @param manifest.setup.container
	 *            {Element} The component's container.
	 * 
	 * @param [manifest.folder]
	 *            {String} The folder in to which to produce image slices. Only
	 *            applies to Ext JS 4.1 (removed in 4.2).
	 * 
	 * @param [manifest.filename]
	 *            {String} The base filename for slices. Only applies to Ext JS
	 *            4.1 (removed in 4.2).
	 * 
	 * @param [manifest.reverse]
	 *            {Boolean} True to position the slices for linear gradient
	 *            background at then opposite "end" (right or bottom) and apply
	 *            the stretch to the area before it (left or top). Only applies
	 *            to Ext JS 4.1 (removed in 4.2).
	 */
	...
	/**
	 * Adds one or more shortcuts to the rendering process. A `shortcut` is an
	 * object that looks the same as a `manifest` entry. These are combined by
	 * copying the properties from the shortcut over those of the manifest
	 * entry. In basic terms:
	 * 
	 * var config = Ext.apply(Ext.apply({}, manfiest.config), shortcut.config);
	 * var entry = Ext.apply(Ext.apply({}, manfiest), shortcut); entry.config =
	 * config;
	 * 
	 * This is not exactly the process, but the idea is the same. The difference
	 * is that the `ui` of the manifest entry is used to replace any `"{ui}"`
	 * substrings found in any string properties of the shortcut or its `config`
	 * object.
	 * 
	 * Usage:
	 * 
	 * Ext.theme.addShortcuts({ 'widget.foo': [{ config: { } },{ config: { } }],
	 * 
	 * 'widget.bar': [ ... ] });
	 */
	...
	/**
	 * This method ensures that a given string has the specified prefix (e.g.,
	 * "widget.").
	 * 
	 * @private
	 */
	...
	/**
	 * This method returns the given string with "widget." added to the front if
	...
	/**
	 * This method accepts an manifest entry and a shortcut entry and returns
	 * the merged version.
	 * 
	 * @private
	 */
	applyShortcut : function(manifestEntry, shortcut) {
		...
		entry.config = config; // both guys have "config" so smash merged one
								// on now...
		return entry;
	},

	/**
	 * This method copies property from a `src` object to a `dest` object and
	 * reaplces `"{foo}"` fragments of any string properties as defined in the
	 * `tpl` object.
	 * 
	 * var obj = Ext.theme.copyProps({}, { foo: 'Hello-{ui}' }, { ui: 'World'
	 * });
	 * 
			...
			for (var key in src) {
				...
				if (tpl && typeof val === 'string') {
					for (var i = 0; i < replacements.length; ++i) {
						val = val.replace(replacements[i].re,
	...
	renderWidget : function(manifestEntry, shortcut) {
		...
		var ct = Ext.fly(document.body).createChild({
		...
		el.addCls('x-slicer-target'); // this is what generateSlicerManifest
										// looks for
	...
	/**
	 * Renders all components (see `render`) and notifies the Slicer that things
	 * are ready.
	 * 
	 * @private
	 */
	run : function() {
		...
		if (Ext.layout.Context) {
			Ext.override(Ext.layout.Context, {
				run : function() {
					var ok = this.callParent(), widget = Ext.theme.currentWidget;
					if (!ok && widget) {
		...
		// Previous to Ext JS 4.2, themes and their manifests where defined
		// differently.
	...
	generateSlicerManifest : function() {
		var now = new Date().getTime(), me = Ext.theme,
		// This function is defined by slicer.js (the framework-independent
		// piece)
		gsm = window && window['generateSlicerManifest'], delta;
		...
		if (gsm) {
			...
			// allow the outer script wrapper a chance to inject the capture
			// function
	...
	/**
	 * Sets the `data-slicer` attribute to the JSON-encoded value of the
	 * provided data.
	 * 
	 * @private
	 */
	...
	loadCss : function(src, callback) {
		...
		xhr.onload = function() {
			var css = xhr.responseText, head = document
