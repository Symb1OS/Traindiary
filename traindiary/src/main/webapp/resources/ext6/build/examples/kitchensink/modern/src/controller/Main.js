 ...
 * 
 * This is an abstract base class that is extended by both the phone and tablet
 * versions. This controller is never directly instantiated, it just provides a
 * set of common functionality that the phone and tablet subclasses both extend.
 */
Ext.define('KitchenSink.controller.Main', {
	extend : 'Ext.app.Controller',

	requires : ['Ext.Deferred'],
	...
	showViewById : function(id) {
		var nav = this.getNav(), view = nav.getStore().getNodeById(id);
	...
	onSourceTap : function() {
		var me = this, overlay = this.getSourceOverlay(), demo = this
				.getCurrentDemo(), view = this.activeView, cls, files, content;
	...
	/**
	 * @private In the kitchen sink we have a large number of dynamic views. If
	 *          we were to keep all of them rendered we'd risk causing the
	 *          browser to run out of memory, especially on older devices. If we
	 *          destroy them as soon as we're done with them, the app can appear
	 *          sluggish. Instead, we keep a small number of rendered views in a
	 *          viewCache so that we can easily reuse recently used views while
	 *          destroying those we haven't used in a while.
	 * @param {String}
	 *            name The full class name of the view to create (e.g.
	 *            "KitchenSink.view.Forms")
	 * @return {Ext.Component} The component, which may be from the cache
	 */
	createView : function(item) {
		var name = this.getViewName(item), cache = this.getViewCache(), ln = cache.length, limit = item
				.get('limit')
				|| 20, view, i = 0, j, oldView;
	...
	/**
	 * @private Returns the full class name of the view to construct for a given
	 *          Demo
	...
	getViewName : function(item) {
		var name = item.get('view') || item.get('text'), ns = 'KitchenSink.view.';
	...
	/**
	 * we iterate over all of the floating sheet components and make sure
	 * they're hidden when we navigate to a new view. This stops things like
	 * Picker overlays staying visible when you hit the browser's back button
	 */
	hideSheets : function() {
		Ext.each(Ext.ComponentQuery.query('sheet, #editorPanel'), function(
						sheet) {
