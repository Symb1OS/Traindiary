Ext.define('KitchenSink.util.Proxy', {
	...
	process : function(url) {
		var speakerStore = Ext.getStore('Speakers'), speakerIds = [], speakerModel;
		...
		Ext.data.JsonP.request({
			...
			success : function(data) {
				Ext.Array.each(data.proposals, function(proposal) {
							Ext.Array.each(proposal.speakers,
									function(speaker) {
										// don't add duplicates or items with no
										// photos.
										if (speakerIds.indexOf(speaker.id) == -1
												&& speaker.photo
												&& speakerIds.length < 25) {
