Ext.define('KitchenSink.view.ColorPatterns', {
	...
	colors : ["#115fa6", "#94ae0a", "#a61120", "#ff8809", "#ffd13e", "#a61187",
	...
	getAlteredBaseColorsHSL : function(deltaHSL) {
		deltaHSL = Ext.applyIf(deltaHSL || {}, {
					h : 0,
					s : 0,
					l : 0
				});
		var deltaH = deltaHSL.h, deltaS = deltaHSL.s, deltaL = deltaHSL.l, colors = [], i, hsl;
		for (i = 0; i < this.colors.length; i++) {
			hsl = Ext.draw.Color.create(this.colors[i]).getHSL();
			colors.push(Ext.draw.Color.fromHSL(hsl[0] + deltaH,
	...
	getGradientColorsHSL : function(baseColor, from, to, number) {
		...
		var hsl = baseColor.getHSL(), fromH = 'h' in from ? from.h : hsl[0], fromS = 's' in from
				? from.s
				: hsl[1], fromL = 'l' in from ? from.l : hsl[2], toH = 'h' in to
				? to.h
				: hsl[0], toS = 's' in to ? to.s : hsl[1], toL = 'l' in to
				? to.l
				: hsl[2], i, colors = [], deltaH = (toH - fromH) / number, deltaS = (toS - fromS)
				/ number, deltaL = (toL - fromL) / number;
		for (i = 0; i <= number; i++) {
			colors.push(Ext.draw.Color.fromHSL(fromH + deltaH * i,
					fromS + deltaS * i, fromL + deltaL * i).toString());
		}
		return colors;
	},

	getGradientColors : function(fromColor, toColor, number) {
		var colors = [], temp = new Ext.draw.Color(), i;
	...
	getGradientColorsByBrightness : function(baseColor, fromBrightness,
			toBrightness, number) {
		baseColor = Ext.draw.Color.create(baseColor);
		...
		for (i = 0; i <= number; i++) {
			colors.push(Ext.draw.Color.fromHSL(
					hsl[0],
					hsl[1],
					fromBrightness * (1 - i / number) + toBrightness * i
							/ number).toString());
		}
		return colors;
	}
