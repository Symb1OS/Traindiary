	...
	Ext.define('KitchenSink.view.FreeDrawComponent', {
		...
		config : {
			...
			listeners : {
				...
				'drag' : function(e) {
					if (sprite) {
						var me = this, p = e.touches[0].point, xy = me.element
								.getXY(), x = p.x - xy[0], y = p.y - xy[1], dx = this.lastEventX
								- x, dy = this.lastEventY - y, D = 40;
						if (dx * dx + dy * dy < D * D) {
							list.length -= 2;
							list.push(p.x - xy[0], p.y - xy[1]);
						} else {
							list.length -= 2;
							list.push(this.lastEventX = p.x - xy[0],
									this.lastEventY = p.y - xy[1]);
				...
				'touchstart' : function(e) {
					if (!sprite) {
						var cmp = this, p0 = cmp.element.getXY(), p = [
								e.pageX - p0[0], e.pageY - p0[1]];
						list = [p[0], p[1], p[0], p[1]];
						this.lastEventX = p[0];
						this.lastEventY = p[1];
						cmp.getSurface('overlay').element.setStyle({
									zIndex : 1
								});
						sprite = cmp.getSurface('overlay').add({
							type : 'path',
							path : ['M', list[0], list[1], 'L', list[0] + 1e-5,
									list[1] + 1e-5],
							lineWidth : 30 * Math.random() + 10,
							lineCap : 'round',
							lineJoin : 'round',
							strokeStyle : new Ext.draw.Color(Math.random()
											* 127 + 128, Math.random() * 127
											+ 128, Math.random() * 127 + 128)
						});
						old1 = old2 = p;
