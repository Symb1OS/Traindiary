/*
 * This panel sets up a DataView, which defines an XTemplate used to render our
 * data. We also set up the toolbar with the "Load Nested Data" button here
 */
Ext.define('KitchenSink.view.NestedLoading', {
	...
	items : [{
		...
		styleHtmlContent : true,
		/*
		 * The XTemplate allows us to easily render the data from our User
		 * model, as well as iterating over each User's Orders and OrderItems:
		 */
		itemTpl : ['<div class="user">', '<h3>{name}\'s orders:</h3>',
				'<tpl for="orders">',
				'<div class="order" style="padding: 0 0 10px 20px;">',
				'Order: {id} ({status})', '<ul>', '<tpl for="orderItems">',
				'<li>{quantity} x {name}</li>', '</tpl>', '</ul>', '</div>',
				'</tpl>', '</div>'].join(''),
		store : {
			model : 'User',
			autoDestroy : true
