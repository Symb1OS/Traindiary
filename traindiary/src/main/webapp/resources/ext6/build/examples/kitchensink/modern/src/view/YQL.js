/**
 * Demonstrates using YQL to fetch data from remote sources (in this case
 * loading from the Sencha blog)
 */
		...
		config : {
			...
			items : [{
						...
						items : [{
							text : 'Load using YQL',
							handler : function() {
								var panel = Ext.getCmp('YQL'), tpl = new Ext.XTemplate([
										'<tpl for="item">',
										'<div class="blog-post">',
								...
								Ext.YQL.request({
									...
									callback : function(success, response) {
										if (success && response.query
												&& response.query.results) {
											panel
													.setHtml(tpl
															.apply(response.query.results));
										} else {
											Ext.Msg
													.alert(
															'Error',
															'There was an error retrieving the YQL request.',
															Ext.emptyFn);
										}
