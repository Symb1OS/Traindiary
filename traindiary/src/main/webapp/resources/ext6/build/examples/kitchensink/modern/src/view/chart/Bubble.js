(function() {
	...
	Ext.define('KitchenSink.view.chart.Bubble', {
		extend : 'Ext.Panel',
		requires : ['Ext.chart.CartesianChart', 'Ext.chart.series.Scatter',
			...
			series : [{
				...
				style : {
					renderer : function(sprite, config, rendererData, index) {
						var store = rendererData.store, storeItem = store
								.getData().items[index];
						config.radius = interpolate(storeItem.data.g3, 0, 1000,
								5, 30);
						config.fillOpacity = interpolate(storeItem.data.g3, 0,
								1000, 1, 0.7);
						config.fill = interpolateColor(storeItem.data.g3, 0,
								1000);
						config.stroke = Ext.draw.Color.fromString(config.fill)
								.createDarker(0.15).toString();
					}
				}
			}],
			axes : [{
	...
	function interpolate(lambda, minSrc, maxSrc, minDst, maxDst) {
		return minDst
				+ (maxDst - minDst)
				* Math.max(0, Math
								.min(1, (lambda - minSrc) / (maxSrc - minSrc)));
	}

	var fromHSL = Ext.draw.Color.fly('blue').getHSL(), toHSL = Ext.draw.Color
			.fly('red').getHSL();
	fromHSL[2] = 0.5;
