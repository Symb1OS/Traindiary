Ext.define('KitchenSink.view.chart.BasicController', {
	...
	onColumnEdit : function(chart, data) {
		var threshold = 65, delta = 20, yValue = data.target.yValue, coldness;
	...
	updateChartTitle : function(viewport, orientation, width, height) {
		var chart = this.lookupReference('chart'), chartSurface = chart
				.getSurface('chart'), title = chartSurface.getItems()[0];
	...
	onAfterRender : function() {
		var me = this, chart = this.lookupReference('chart'), axis = chart
				.getAxis(0), store = chart.getStore();
	...
	onAxisRangeChange : function(axis, range) {
		...
		var chart = axis.getChart(), store = chart.getStore(), sum = 0, mean;
	...
	onBeginItemEdit : function(chart, interaction, item) {
		var itemsMarker = item.sprite.getMarker(item.category), fx = itemsMarker
				.getTemplate().fx; // animation modifier
