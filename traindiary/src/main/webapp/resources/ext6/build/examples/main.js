window.onload = function() {
	var html = '', groups = Ext.samples.samplesCatalog, ln = groups.length, i = 0, groupIndex = 0, bodyEl = document
			.getElementById('body'), group, example, j, examples, expanded, exampleLn;
	...
	for (; i < ln; i++) {
		...
		'<div class="group-header ' + (expanded ? 'expanded' : 'collapsed')
				+ '">' + '<div class="wrap">' + '<div class="group-title">'
				+ group.title + '</div>' + '<div class="group-action">'
				+ '<a href="#" class="group-action-icon icon-'
				+ (expanded ? 'minus' : 'plus') + '"></a>' + '</div>'
		...
		for (j = 0; j < exampleLn; j++) {
			example = examples[j];
			html += '<a class="example" target="_blank" href="' + example.url
					+ '">'
					+ '<div class="example-icon-wrap icon-border-hexagon">'
					+ '<div class="example-icon icon-' + example.icon
					+ '"></div>' + '</div>' + '<div class="example-text-wrap">'
					+ '<div class="example-text-wrap-inner">'
					+ '<div class="example-title">' + example.text + '</div>'
					+ '<div class="example-description">' + example.desc
					+ '</div>' + '</div>' + '</div>' + '</a>';
		}
	...
	addListener(document.body, 'click', function(e) {
		var target = e.target || e.srcElement, groupHeaderClicked = false, expander, className, expanderClassName;

		while (target) {
			if (target.className
					&& target.className.indexOf('group-header') !== -1) {
		...
		if (groupHeaderClicked) {
			...
			// IE8 needs a repaint of the body el to trigger the stylesheet
			// rules that hide
			// and show the group
